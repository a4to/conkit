...
import (
	...
	"github.com/a4to/mageutil"
	_ "github.com/a4to/psrpc"
)

const (
	goChecksumFile = ".checksumgo"
	imageName      = "conkit/conkit-server"
)
...
func Build() error {
	...
	if err := mageutil.RunDir(context.Background(), "cmd/server", "go build -o ../../bin/conkit-server"); err != nil {
		return err
	}
...
func BuildLinux() error {
	...
	cmd := mageutil.CommandDir(context.Background(), "cmd/server", "go build -buildvcs=false -o ../../bin/conkit-server-amd64")
	cmd.Env = []string{
		"GOOS=linux",
		"GOARCH=amd64",
		"HOME=" + os.Getenv("HOME"),
