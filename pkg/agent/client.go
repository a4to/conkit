...
import (
	...
	serverutils "github.com/a4to/conkit-server/pkg/utils"
...
var jobTypeTopics = map[conkit.JobType]string{
	conkit.JobType_JT_ROOM:        RoomAgentTopic,
	conkit.JobType_JT_PUBLISHER:   PublisherAgentTopic,
	conkit.JobType_JT_PARTICIPANT: ParticipantAgentTopic,
}

type Client interface {
	// LaunchJob starts a room or participant job on an agent.
	// it will launch a job once for each worker in each namespace
	LaunchJob(ctx context.Context, desc *JobRequest) *serverutils.IncrementalDispatcher[*conkit.Job]
	TerminateJob(ctx context.Context, jobID string, reason rpc.JobTerminateReason) (*conkit.JobState, error)
	Stop() error
}
...
func (c *agentClient) LaunchJob(ctx context.Context, desc *JobRequest) *serverutils.IncrementalDispatcher[*conkit.Job] {
	var wg sync.WaitGroup
	ret := serverutils.NewIncrementalDispatcher[*conkit.Job]()
	defer func() {
...
func (c *agentClient) TerminateJob(ctx context.Context, jobID string, reason rpc.JobTerminateReason) (*conkit.JobState, error) {
...
func (c *agentClient) getDispatcher(agName string, jobType conkit.JobType) *serverutils.IncrementalDispatcher[string] {
	c.mu.Lock()
