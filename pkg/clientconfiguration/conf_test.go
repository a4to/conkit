...
func TestScriptMatchConfiguration(t *testing.T) {
	t.Run("no merge", func(t *testing.T) {
		confs := []ConfigurationItem{
			{
				...
				Configuration: &conkit.ClientConfiguration{
					ResumeConnection: conkit.ClientConfigSetting_ENABLED,
				},
			},
		}
		...
		conf := cm.GetConfiguration(&conkit.ClientInfo{Protocol: 4})
		require.Nil(t, conf)

		conf = cm.GetConfiguration(&conkit.ClientInfo{Protocol: 6, Browser: "firefox"})
		require.Nil(t, conf)

		conf = cm.GetConfiguration(&conkit.ClientInfo{Protocol: 6, Browser: "chrome"})
		require.Equal(t, conf.ResumeConnection, conkit.ClientConfigSetting_ENABLED)
	})
	...
	t.Run("merge", func(t *testing.T) {
		confs := []ConfigurationItem{
			...
			},
			{
				...
				Configuration: &conkit.ClientConfiguration{
					Video: &conkit.VideoConfiguration{
						HardwareEncoder: conkit.ClientConfigSetting_DISABLED,
					},
				},
				Merge: true,
			},
		...
		conf = cm.GetConfiguration(&conkit.ClientInfo{Protocol: 6, Browser: "chrome", Sdk: 3})
		require.Equal(t, conf.ResumeConnection, conkit.ClientConfigSetting_ENABLED)
		require.Equal(t, conf.Video.HardwareEncoder, conkit.ClientConfigSetting_DISABLED)
	})
}
