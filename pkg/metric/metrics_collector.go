...
type MetricsCollectorProvider interface {
	...
	MetricsCollectorBatchReady(mb *conkit.MetricsBatch)
}
...
type MetricsCollector struct {
	...
	publisherRTTMetricId  map[conkit.ParticipantIdentity]int
	subscriberRTTMetricId int
	relayRTTMetricId      map[conkit.ParticipantIdentity]int
...
func (mc *MetricsCollector) AddPublisherRTT(participantIdentity conkit.ParticipantIdentity, rtt float32) {
	mc.lock.Lock()
	...
	if !ok {
		var err error
		metricId, err = mc.createTimeSeriesMetric(conkit.MetricLabel_PUBLISHER_RTT, participantIdentity)
		if err != nil {
...
func (mc *MetricsCollector) AddSubscriberRTT(rtt float32) {
	...
	if mc.subscriberRTTMetricId == utils.MetricsBatchBuilderInvalidTimeSeriesMetricId {
		var err error
		mc.subscriberRTTMetricId, err = mc.createTimeSeriesMetric(conkit.MetricLabel_SUBSCRIBER_RTT, mc.params.ParticipantIdentity)
		if err != nil {
			mc.params.Logger.Warnw("could not add time series metric for publisher RTT", err)
			return
		}
...
func (mc *MetricsCollector) AddRelayRTT(participantIdentity conkit.ParticipantIdentity, rtt float32) {
	mc.lock.Lock()
	defer mc.lock.Unlock()
	...
	if !ok {
		var err error
		metricId, err = mc.createTimeSeriesMetric(conkit.MetricLabel_SERVER_MESH_RTT, participantIdentity)
		if err != nil {
...
func (mc *MetricsCollector) getMetricsBatchAndReset() *conkit.MetricsBatch {
	mc.lock.Lock()
	mbb := mc.mbb
...
func (mc *MetricsCollector) reset() {
	...
	mc.mbb.SetRestrictedLabels(utils.MetricRestrictedLabels{
		LabelRanges: []utils.MetricLabelRange{
			{
				StartInclusive: conkit.MetricLabel_CLIENT_VIDEO_SUBSCRIBER_FREEZE_COUNT,
				EndInclusive:   conkit.MetricLabel_CLIENT_VIDEO_PUBLISHER_QUALITY_LIMITATION_DURATION_OTHER,
			},
		},
		ParticipantIdentity: mc.params.ParticipantIdentity,
	})

	mc.publisherRTTMetricId = make(map[conkit.ParticipantIdentity]int)
	mc.subscriberRTTMetricId = utils.MetricsBatchBuilderInvalidTimeSeriesMetricId
	mc.relayRTTMetricId = make(map[conkit.ParticipantIdentity]int)
}

func (mc *MetricsCollector) createTimeSeriesMetric(
	label conkit.MetricLabel,
	participantIdentity conkit.ParticipantIdentity,
) (int, error) {
