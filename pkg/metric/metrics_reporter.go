...
type MetricsReporterConsumer interface {
	MetricsReporterBatchReady(mb *conkit.MetricsBatch)
}
...
func (mr *MetricsReporter) Merge(other *conkit.MetricsBatch) {
	if mr == nil {
		return
	}
...
func (mr *MetricsReporter) getMetricsBatchAndReset() *conkit.MetricsBatch {
	mr.lock.Lock()
	mbb := mr.mbb
