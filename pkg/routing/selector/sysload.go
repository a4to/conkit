...
func (s *SystemLoadSelector) filterNodes(nodes []*conkit.Node) ([]*conkit.Node, error) {
	nodes = GetAvailableNodes(nodes)
	if len(nodes) == 0 {
		return nil, ErrNoAvailableNodes
	}

	nodesLowLoad := make([]*conkit.Node, 0)
...
func (s *SystemLoadSelector) SelectNode(nodes []*conkit.Node) (*conkit.Node, error) {
