...
func IsAvailable(node *conkit.Node) bool {
	if node.Stats == nil {
...
func GetAvailableNodes(nodes []*conkit.Node) []*conkit.Node {
	return funk.Filter(nodes, func(node *conkit.Node) bool {
		return IsAvailable(node) && node.State == conkit.NodeState_SERVING
	}).([]*conkit.Node)
}

func GetNodeSysload(node *conkit.Node) float32 {
	stats := node.Stats
...
func LimitsReached(limitConfig config.LimitConfig, nodeStats *conkit.NodeStats) bool {
	if nodeStats == nil {
		return false
	}
...
func SelectSortedNode(nodes []*conkit.Node, sortBy string) (*conkit.Node, error) {
	if sortBy == "" {
