...
type SignalClient interface {
	ActiveCount() int
	StartParticipantSignal(ctx context.Context, roomName conkit.RoomName, pi ParticipantInit, nodeID conkit.NodeID) (connectionID conkit.ConnectionID, reqSink MessageSink, resSource MessageSource, err error)
}
...
func NewSignalClient(nodeID conkit.NodeID, bus psrpc.MessageBus, config config.SignalRelayConfig) (SignalClient, error) {
...
func (r *signalClient) StartParticipantSignal(
	...
	connectionID = conkit.ConnectionID(guid.New("CO_"))
...
func (e signalRequestMessageWriter) Write(seq uint64, close bool, msgs []proto.Message) *rpc.RelaySignalRequest {
	r := &rpc.RelaySignalRequest{
		Seq:      seq,
		Requests: make([]*conkit.SignalRequest, 0, len(msgs)),
		Close:    close,
	}
	for _, m := range msgs {
		r.Requests = append(r.Requests, m.(*conkit.SignalRequest))
	}
	return r
}
...
func (s *signalMessageSink[SendType, RecvType]) ConnectionID() conkit.ConnectionID {
