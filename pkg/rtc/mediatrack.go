...
import (
	...
	"github.com/a4to/conkit-server/pkg/rtc/dynacast"
	...
	util "github.com/a4to/mediatransportutil"
)
...
type MediaTrackParams struct {
	...
	OnTrackEverSubscribed func(conkit.TrackID)
}

func NewMediaTrack(params MediaTrackParams, ti *conkit.TrackInfo) *MediaTrack {
	t := &MediaTrack{
		params: params,
	}

	// TODO: disable codec regression until simulcast-codec clients knows that
	if ti.BackupCodecPolicy == conkit.BackupCodecPolicy_REGRESSION && len(ti.Codecs) > 1 {
	...
	if ti.Type == conkit.TrackType_AUDIO {
	...
	if ti.Type == conkit.TrackType_VIDEO {
		...
		t.MediaTrackReceiver.OnSubscriberMaxQualityChange(
			func(subscriberID conkit.ParticipantID, mimeType mime.MimeType, layer int32) {
...
func (t *MediaTrack) OnSubscribedMaxQualityChange(
	...
	handler := func(subscribedQualities []*conkit.SubscribedCodec, maxSubscribedQualities []types.SubscribedCodecQuality) {
		if f != nil && !t.IsMuted() {
...
func (t *MediaTrack) NotifySubscriberNodeMaxQuality(nodeID conkit.NodeID, qualities []types.SubscribedCodecQuality) {
	if t.dynacastManager != nil {
...
func (t *MediaTrack) ToProto() *conkit.TrackInfo {
	return t.MediaTrackReceiver.TrackInfoClone()
}

func (t *MediaTrack) UpdateCodecCid(codecs []*conkit.SimulcastCodec) {
...
func (t *MediaTrack) AddReceiver(receiver *webrtc.RTPReceiver, track sfu.TrackRemote, mid string) bool {
	...
	if wr == nil {
		...
		if priority == 0 {
			newWR.OnStatsUpdate(func(_ *sfu.WebRTCReceiver, stat *conkit.AnalyticsStat) {
				key := telemetry.StatsKeyForTrack(conkit.StreamType_UPSTREAM, t.PublisherID(), t.ID(), ti.Source, ti.Type)
	...
	buff.OnFinalRtpStats(func(stats *conkit.RTPStats) {
...
func (t *MediaTrack) GetConnectionScoreAndQuality() (float32, conkit.ConnectionQuality) {
	...
	return connectionquality.MaxMOS, conkit.ConnectionQuality_EXCELLENT
}
