...
import (
	...
	"github.com/a4to/conkit-server/pkg/sfu/rtpstats"
...
type MediaTrackReceiver struct {
	...
	trackInfo          atomic.Pointer[conkit.TrackInfo]
...
func NewMediaTrackReceiver(params MediaTrackReceiverParams, ti *conkit.TrackInfo) *MediaTrackReceiver {
...
func (t *MediaTrackReceiver) Restart() {
	hq := buffer.VideoQualityToSpatialLayer(conkit.VideoQuality_HIGH, t.TrackInfo())
...
func (t *MediaTrackReceiver) ID() conkit.TrackID {
	return conkit.TrackID(t.TrackInfo().Sid)
}

func (t *MediaTrackReceiver) Kind() conkit.TrackType {
	return t.TrackInfo().Type
}

func (t *MediaTrackReceiver) Source() conkit.TrackSource {
...
func (t *MediaTrackReceiver) PublisherID() conkit.ParticipantID {
	return t.params.ParticipantID
}

func (t *MediaTrackReceiver) PublisherIdentity() conkit.ParticipantIdentity {
...
func (t *MediaTrackReceiver) IsEncrypted() bool {
	return t.TrackInfo().Encryption != conkit.Encryption_NONE
}
...
func (t *MediaTrackReceiver) RemoveSubscriber(subscriberID conkit.ParticipantID, isExpectedToResume bool) {
...
func (t *MediaTrackReceiver) RevokeDisallowedSubscribers(allowedSubscriberIdentities []conkit.ParticipantIdentity) []conkit.ParticipantIdentity {
	var revokedSubscriberIdentities []conkit.ParticipantIdentity
...
func (t *MediaTrackReceiver) SetLayerSsrc(mimeType mime.MimeType, rid string, ssrc uint32) {
	...
	for i, ci := range trackInfo.Codecs {
		...
		var matchingLayer *conkit.VideoLayer
		ssrcFound := false
...
func (t *MediaTrackReceiver) UpdateCodecCid(codecs []*conkit.SimulcastCodec) {
	t.lock.Lock()
	trackInfo := t.TrackInfoClone()
...
func (t *MediaTrackReceiver) UpdateTrackInfo(ti *conkit.TrackInfo) {
	updateMute := false
...
func (t *MediaTrackReceiver) UpdateAudioTrack(update *conkit.UpdateLocalAudioTrack) {
	if t.Kind() != conkit.TrackType_AUDIO {
		return
	}
...
func (t *MediaTrackReceiver) UpdateVideoTrack(update *conkit.UpdateLocalVideoTrack) {
	if t.Kind() != conkit.TrackType_VIDEO {
		return
	}
...
func (t *MediaTrackReceiver) TrackInfo() *conkit.TrackInfo {
	return t.trackInfo.Load()
}

func (t *MediaTrackReceiver) TrackInfoClone() *conkit.TrackInfo {
...
func (t *MediaTrackReceiver) NotifyMaxLayerChange(maxLayer int32) {
	...
	ti := &conkit.TrackInfo{
		Sid:    trackInfo.Sid,
		Type:   trackInfo.Type,
		Layers: []*conkit.VideoLayer{{Quality: quality}},
	}
...
func (t *MediaTrackReceiver) GetQualityForDimension(width, height uint32) conkit.VideoQuality {
...
func (t *MediaTrackReceiver) GetTrackStats() *conkit.RTPStats {
	receivers := t.loadReceivers()
	stats := make([]*conkit.RTPStats, 0, len(receivers))
	for _, receiver := range receivers {
