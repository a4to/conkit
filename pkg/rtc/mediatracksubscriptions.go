...
type MediaTrackSubscriptions struct {
	...
	subscribedTracks   map[conkit.ParticipantID]types.SubscribedTrack

	onDownTrackCreated           func(downTrack *sfu.DownTrack)
	onSubscriberMaxQualityChange func(subscriberID conkit.ParticipantID, mime mime.MimeType, layer int32)
}
...
func NewMediaTrackSubscriptions(params MediaTrackSubscriptionsParams) *MediaTrackSubscriptions {
	return &MediaTrackSubscriptions{
		params:           params,
		subscribedTracks: make(map[conkit.ParticipantID]types.SubscribedTrack),
	}
}
...
func (t *MediaTrackSubscriptions) OnSubscriberMaxQualityChange(f func(subscriberID conkit.ParticipantID, mime mime.MimeType, layer int32)) {
...
func (t *MediaTrackSubscriptions) IsSubscriber(subID conkit.ParticipantID) bool {
...
func (t *MediaTrackSubscriptions) AddSubscriber(sub types.LocalParticipant, wr *WrappedReceiver) (types.SubscribedTrack, error) {
	...
	downTrack.OnCodecNegotiated(func(codec webrtc.RTPCodecCapability) {
		if !wr.DetermineReceiver(codec) {
			if t.onSubscriberMaxQualityChange != nil {
				go func() {
					spatial := buffer.VideoQualityToSpatialLayer(conkit.VideoQuality_HIGH, t.params.MediaTrack.ToProto())
	...
	downTrack.OnStatsUpdate(func(_ *sfu.DownTrack, stat *conkit.AnalyticsStat) {
		key := telemetry.StatsKeyForTrack(conkit.StreamType_DOWNSTREAM, subscriberID, trackID, t.params.MediaTrack.Source(), t.params.MediaTrack.Kind())
...
func (t *MediaTrackSubscriptions) RemoveSubscriber(subscriberID conkit.ParticipantID, isExpectedToResume bool) error {
...
func (t *MediaTrackSubscriptions) GetAllSubscribers() []conkit.ParticipantID {
	t.subscribedTracksMu.RLock()
	defer t.subscribedTracksMu.RUnlock()

	subs := make([]conkit.ParticipantID, 0, len(t.subscribedTracks))
...
func (t *MediaTrackSubscriptions) GetAllSubscribersForMime(mime mime.MimeType) []conkit.ParticipantID {
	t.subscribedTracksMu.RLock()
	defer t.subscribedTracksMu.RUnlock()
...
func (t *MediaTrackSubscriptions) getSubscribedTrack(subscriberID conkit.ParticipantID) types.SubscribedTrack {
	t.subscribedTracksMu.RLock()
	defer t.subscribedTracksMu.RUnlock()
