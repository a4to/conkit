...
import (
	...
	"github.com/a4to/mediatransportutil/pkg/twcc"
	...
	"github.com/a4to/conkit-server/pkg/metric"
	"github.com/a4to/conkit-server/pkg/routing"
	"github.com/a4to/conkit-server/pkg/rtc/supervisor"
	"github.com/a4to/conkit-server/pkg/rtc/transport"
	...
	"github.com/a4to/conkit-server/pkg/sfu/connectionquality"
	"github.com/a4to/conkit-server/pkg/sfu/mime"
	"github.com/a4to/conkit-server/pkg/sfu/pacer"
	"github.com/a4to/conkit-server/pkg/sfu/streamallocator"
...
type ParticipantParams struct {
	...
	PublishEnabledCodecs           []*conkit.Codec
	SubscribeEnabledCodecs         []*conkit.Codec
	...
	GetRegionSettings              func(ip string) *conkit.RegionSettings
	GetSubscriberForwarderState    func(p types.LocalParticipant) (map[conkit.TrackID]*conkit.RTPForwarderState, error)
	...
	PlayoutDelay                   *conkit.PlayoutDelay
	SyncStreams                    bool
...
type ParticipantImpl struct {
	...
	state        atomic.Value // conkit.ParticipantInfo_State
	...
	pendingPublishingTracks map[conkit.TrackID]*pendingTrackInfo
	pendingRemoteTracks     []*pendingRemoteTrack

	// supported codecs
	enabledPublishCodecs   []*conkit.Codec
	enabledSubscribeCodecs []*conkit.Codec
	...
	*SubscriptionManager
	...
	queuedUpdates []*conkit.ParticipantInfo
	// cache of recently sent updates, to ensuring ordering by version
	// guarded by updateLock
	updateCache *lru.Cache[conkit.ParticipantID, participantUpdateInfo]
	...
	onStateChange        func(p types.LocalParticipant, state conkit.ParticipantInfo_State)
	...
	onDataPacket         func(types.LocalParticipant, conkit.DataPacket_Kind, *conkit.DataPacket)
	onMetrics            func(types.Participant, *conkit.DataPacket)

	migrateState atomic.Value // types.MigrateState

	onClose            func(types.LocalParticipant)
	onClaimsChanged    func(participant types.LocalParticipant)
	onICEConfigChanged func(participant types.LocalParticipant, iceConfig *conkit.ICEConfig)

	cachedDownTracks map[conkit.TrackID]*downTrackState
	forwarderState   map[conkit.TrackID]*conkit.RTPForwarderState

	supervisor *supervisor.ParticipantSupervisor

	connectionQuality conkit.ConnectionQuality
...
func NewParticipant(params ParticipantParams) (*ParticipantImpl, error) {
	...
	p := &ParticipantImpl{
		...
		pendingPublishingTracks: make(map[conkit.TrackID]*pendingTrackInfo),
		connectedAt:             time.Now().Truncate(time.Millisecond),
		rttUpdatedAt:            time.Now(),
		cachedDownTracks:        make(map[conkit.TrackID]*downTrackState),
		...
		connectionQuality: conkit.ConnectionQuality_EXCELLENT,
	...
	p.state.Store(conkit.ParticipantInfo_JOINING)
	...
	if p.updateCache, err = lru.New[conkit.ParticipantID, participantUpdateInfo](128); err != nil {
		return nil, err
	}
...
func (p *ParticipantImpl) ID() conkit.ParticipantID {
	return p.params.SID
}

func (p *ParticipantImpl) Identity() conkit.ParticipantIdentity {
	return p.params.Identity
}

func (p *ParticipantImpl) State() conkit.ParticipantInfo_State {
	return p.state.Load().(conkit.ParticipantInfo_State)
}

func (p *ParticipantImpl) Kind() conkit.ParticipantInfo_Kind {
	return p.grants.Load().GetParticipantKind()
}

func (p *ParticipantImpl) IsRecorder() bool {
	grants := p.grants.Load()
	return grants.GetParticipantKind() == conkit.ParticipantInfo_EGRESS || grants.Video.Recorder
}

func (p *ParticipantImpl) IsAgent() bool {
	grants := p.grants.Load()
	return grants.GetParticipantKind() == conkit.ParticipantInfo_AGENT || grants.Video.Agent
}

func (p *ParticipantImpl) IsDependent() bool {
	grants := p.grants.Load()
	switch grants.GetParticipantKind() {
	case conkit.ParticipantInfo_AGENT, conkit.ParticipantInfo_EGRESS:
		return true
	default:
...
func (p *ParticipantImpl) IsReady() bool {
	...
	if p.params.Migration {
		return state != conkit.ParticipantInfo_DISCONNECTED
	}

	// when not migrating, there is a JoinResponse, state transitions from JOINING -> JOINED -> ACTIVE -> DISCONNECTED
	return state == conkit.ParticipantInfo_JOINED || state == conkit.ParticipantInfo_ACTIVE
}

func (p *ParticipantImpl) IsDisconnected() bool {
	return p.State() == conkit.ParticipantInfo_DISCONNECTED
}
...
func (p *ParticipantImpl) GetClientInfo() *conkit.ClientInfo {
	p.lock.RLock()
	defer p.lock.RUnlock()
	return p.params.ClientInfo.ClientInfo
}

func (p *ParticipantImpl) GetClientConfiguration() *conkit.ClientConfiguration {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
func (p *ParticipantImpl) SetPermission(permission *conkit.ParticipantPermission) bool {
	if permission == nil {
		return false
	}
	p.lock.Lock()
...
func (p *ParticipantImpl) ToProtoWithVersion() (*conkit.ParticipantInfo, utils.TimedVersion) {
	p.maybeIncVersion()
...
func (p *ParticipantImpl) ToProto() *conkit.ParticipantInfo {
...
func (p *ParticipantImpl) OnStateChange(callback func(p types.LocalParticipant, state conkit.ParticipantInfo_State)) {
	p.lock.Lock()
	p.onStateChange = callback
	p.lock.Unlock()
}

func (p *ParticipantImpl) getOnStateChange() func(p types.LocalParticipant, state conkit.ParticipantInfo_State) {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
func (p *ParticipantImpl) OnDataPacket(callback func(types.LocalParticipant, conkit.DataPacket_Kind, *conkit.DataPacket)) {
	p.lock.Lock()
	p.onDataPacket = callback
	p.lock.Unlock()
}

func (p *ParticipantImpl) getOnDataPacket() func(types.LocalParticipant, conkit.DataPacket_Kind, *conkit.DataPacket) {
	p.lock.RLock()
	defer p.lock.RUnlock()
	return p.onDataPacket
}

func (p *ParticipantImpl) OnMetrics(callback func(types.Participant, *conkit.DataPacket)) {
	p.lock.Lock()
	p.onMetrics = callback
	p.lock.Unlock()
}

func (p *ParticipantImpl) getOnMetrics() func(types.Participant, *conkit.DataPacket) {
	p.lock.RLock()
	defer p.lock.RUnlock()
	return p.onMetrics
}
...
func (p *ParticipantImpl) HandleOffer(offer webrtc.SessionDescription) error {
	p.pubLogger.Debugw("received offer", "transport", conkit.SignalTarget_PUBLISHER, "offer", offer)
...
func (p *ParticipantImpl) onPublisherAnswer(answer webrtc.SessionDescription) error {
	...
	p.pubLogger.Debugw("sending answer", "transport", conkit.SignalTarget_PUBLISHER, "answer", answer)
	return p.writeMessage(&conkit.SignalResponse{
		Message: &conkit.SignalResponse_Answer{
...
func (p *ParticipantImpl) GetAnswer() (webrtc.SessionDescription, error) {
	...
	answer = p.configurePublisherAnswer(answer)
	p.pubLogger.Debugw("returning answer", "transport", conkit.SignalTarget_PUBLISHER, "answer", answer)
	return answer, nil
}
...
func (p *ParticipantImpl) HandleAnswer(answer webrtc.SessionDescription) {
	p.subLogger.Debugw("received answer", "transport", conkit.SignalTarget_SUBSCRIBER, "answer", answer)
...
func (p *ParticipantImpl) AddTrack(req *conkit.AddTrackRequest) {
...
func (p *ParticipantImpl) SetMigrateInfo(
	...
	for _, t := range mediaTracks {
		...
		if p.supervisor != nil {
			p.supervisor.AddPublication(conkit.TrackID(ti.Sid))
			p.supervisor.SetPublicationMute(conkit.TrackID(ti.Sid), ti.Muted)
		}

		p.pendingTracks[t.GetCid()] = &pendingTrackInfo{trackInfos: []*conkit.TrackInfo{ti}, migrated: true, createdAt: time.Now()}
...
func (p *ParticipantImpl) Close(sendLeave bool, reason types.ParticipantCloseReason, isExpectedToResume bool) error {
	...
	p.pendingPublishingTracks = make(map[conkit.TrackID]*pendingTrackInfo)
	p.pendingTracksLock.Unlock()

	p.UpTrackManager.Close(isExpectedToResume)

	p.updateState(conkit.ParticipantInfo_DISCONNECTED)
...
func (p *ParticipantImpl) ICERestart(iceConfig *conkit.ICEConfig) {
	if p.params.UseOneShotSignallingMode {
		return
	}
...
func (p *ParticipantImpl) OnICEConfigChanged(f func(participant types.LocalParticipant, iceConfig *conkit.ICEConfig)) {
	p.lock.Lock()
	p.onICEConfigChanged = f
	p.lock.Unlock()
}

func (p *ParticipantImpl) GetConnectionQuality() *conkit.ConnectionQualityInfo {
	minQuality := conkit.ConnectionQuality_EXCELLENT
	...
	if minQuality == conkit.ConnectionQuality_LOST && !p.ProtocolVersion().SupportsConnectionQualityLost() {
		minQuality = conkit.ConnectionQuality_POOR
	}
	...
	return &conkit.ConnectionQualityInfo{
...
func (p *ParticipantImpl) CanPublishSource(source conkit.TrackSource) bool {
...
func (p *ParticipantImpl) Verify() bool {
	state := p.State()
	isActive := state != conkit.ParticipantInfo_JOINING && state != conkit.ParticipantInfo_JOINED
	if p.params.UseOneShotSignallingMode {
...
func (p *ParticipantImpl) VerifySubscribeParticipantInfo(pID conkit.ParticipantID, version uint32) {
	if !p.IsReady() {
	...
	if f := p.params.GetParticipantInfo; f != nil {
		if info := f(pID); info != nil {
			_ = p.SendParticipantUpdate([]*conkit.ParticipantInfo{info})
		}
	}
}
...
func (p *ParticipantImpl) SubscriptionPermissionUpdate(publisherID conkit.ParticipantID, trackID conkit.TrackID, allowed bool) {
	...
	err := p.writeMessage(&conkit.SignalResponse{
		Message: &conkit.SignalResponse_SubscriptionPermissionUpdate{
			SubscriptionPermissionUpdate: &conkit.SubscriptionPermissionUpdate{
...
func (h AnyTransportHandler) OnICECandidate(c *webrtc.ICECandidate, target conkit.SignalTarget) error {
...
func (h PublisherTransportHandler) OnDataPacket(kind conkit.DataPacket_Kind, data []byte) {
...
func (p *ParticipantImpl) setupTransportManager() error {
	...
	tm.OnICEConfigChanged(func(iceConfig *conkit.ICEConfig) {
		p.lock.Lock()
		onICEConfigChanged := p.onICEConfigChanged

		if p.params.ClientConf == nil {
			p.params.ClientConf = &conkit.ClientConfiguration{}
		}
		if iceConfig.PreferenceSubscriber == conkit.ICECandidateType_ICT_TLS {
			p.params.ClientConf.ForceRelay = conkit.ClientConfigSetting_ENABLED
		} else {
			// UNSET indicates that clients could override RTCConfiguration to forceRelay
			p.params.ClientConf.ForceRelay = conkit.ClientConfigSetting_UNSET
		}
		p.lock.Unlock()
...
func (p *ParticipantImpl) MetricsCollectorBatchReady(mb *conkit.MetricsBatch) {
	if onMetrics := p.getOnMetrics(); onMetrics != nil {
		onMetrics(p, &conkit.DataPacket{
			ParticipantIdentity: string(p.Identity()),
			Value: &conkit.DataPacket_Metrics{
				Metrics: mb,
			},
		})
	}
...
func (p *ParticipantImpl) MetricsReporterBatchReady(mb *conkit.MetricsBatch) {
	dpData, err := proto.Marshal(&conkit.DataPacket{
		ParticipantIdentity: string(p.Identity()),
		Value: &conkit.DataPacket_Metrics{
			Metrics: mb,
		},
	...
	p.TransportManager.SendDataPacket(conkit.DataPacket_RELIABLE, dpData)
}
...
func (p *ParticipantImpl) updateState(state conkit.ParticipantInfo_State) {
	if state == conkit.ParticipantInfo_ACTIVE {
		t := time.Now()
		p.lastActiveAt.CompareAndSwap(nil, &t)
	}
	oldState := p.state.Swap(state).(conkit.ParticipantInfo_State)
	if oldState == state {
		return
	}

	if state == conkit.ParticipantInfo_DISCONNECTED && oldState == conkit.ParticipantInfo_ACTIVE {
...
func (p *ParticipantImpl) setIsPublisher(isPublisher bool) {
	...
	if p.State() == conkit.ParticipantInfo_ACTIVE {
		p.lock.RLock()
		onParticipantUpdate := p.onParticipantUpdate
		p.lock.RUnlock()
...
func (p *ParticipantImpl) onSubscriberOffer(offer webrtc.SessionDescription) error {
	p.subLogger.Debugw("sending offer", "transport", conkit.SignalTarget_SUBSCRIBER, "offer", offer)
	return p.writeMessage(&conkit.SignalResponse{
		Message: &conkit.SignalResponse_Offer{
...
func (p *ParticipantImpl) onDataMessage(kind conkit.DataPacket_Kind, data []byte) {
	...
	case *conkit.DataPacket_SipDtmf:
		if payload.SipDtmf == nil {
			return
		}
	case *conkit.DataPacket_Transcription:
	...
	case *conkit.DataPacket_ChatMessage:
	...
	case *conkit.DataPacket_Metrics:
	...
	case *conkit.DataPacket_RpcRequest:
	...
	case *conkit.DataPacket_RpcResponse:
		if payload.RpcResponse == nil {
			return
		}
	case *conkit.DataPacket_RpcAck:
		if payload.RpcAck == nil {
			return
		}
	case *conkit.DataPacket_StreamHeader:
		...
		if p.IsAgent() && dp.ParticipantIdentity != "" && string(p.params.Identity) != dp.ParticipantIdentity {
			...
			case *conkit.DataStream_Header_TextHeader:
	...
	case *conkit.DataPacket_StreamChunk:
		if payload.StreamChunk == nil {
			return
		}
	case *conkit.DataPacket_StreamTrailer:
...
func (p *ParticipantImpl) onICECandidate(c *webrtc.ICECandidate, target conkit.SignalTarget) error {
	if p.IsDisconnected() || p.IsClosed() {
		return nil
	}

	if target == conkit.SignalTarget_SUBSCRIBER && p.MigrateState() == types.MigrateStateInit {
		return nil
	}
...
func (p *ParticipantImpl) onStreamStateChange(update *streamallocator.StreamStateUpdate) error {
	...
	streamStateUpdate := &conkit.StreamStateUpdate{}
	for _, streamStateInfo := range update.StreamStates {
		state := conkit.StreamState_ACTIVE
		if streamStateInfo.State == streamallocator.StreamStatePaused {
			state = conkit.StreamState_PAUSED
		}
		streamStateUpdate.StreamStates = append(streamStateUpdate.StreamStates, &conkit.StreamStateInfo{
	...
	return p.writeMessage(&conkit.SignalResponse{
		Message: &conkit.SignalResponse_StreamStateUpdate{
...
func (p *ParticipantImpl) onSubscribedMaxQualityChange(
	...
	subscribedQualities []*conkit.SubscribedCodec,
	...
	subscribedQualityUpdate := &conkit.SubscribedQualityUpdate{
		TrackSid:            string(trackID),
	...
	return p.writeMessage(&conkit.SignalResponse{
		Message: &conkit.SignalResponse_SubscribedQualityUpdate{
			SubscribedQualityUpdate: subscribedQualityUpdate,
		},
	})
}

func (p *ParticipantImpl) addPendingTrackLocked(req *conkit.AddTrackRequest) *conkit.TrackInfo {
	if req.Sid != "" {
		track := p.GetPublishedTrack(conkit.TrackID(req.Sid))
		if track == nil {
	...
	if backupCodecPolicy == conkit.BackupCodecPolicy_REGRESSION && p.params.DisableCodecRegression {
		backupCodecPolicy = conkit.BackupCodecPolicy_SIMULCAST
	}
	...
	if req.Stereo {
		ti.AudioFeatures = append(ti.AudioFeatures, conkit.AudioTrackFeature_TF_STEREO)
	}
	if req.DisableDtx {
		ti.AudioFeatures = append(ti.AudioFeatures, conkit.AudioTrackFeature_TF_NO_DTX)
	}
	if ti.Stream == "" {
	...
	if len(req.SimulcastCodecs) == 0 {
		if req.Type == conkit.TrackType_VIDEO {
			// clients not supporting simulcast codecs, synthesise a codec
			ti.Codecs = append(ti.Codecs, &conkit.SimulcastCodecInfo{
				Cid:    req.Cid,
				Layers: req.Layers,
			})
		}
		...
		for _, codec := range req.SimulcastCodecs {
			...
			} else if req.Type == conkit.TrackType_AUDIO && !mime.IsMimeTypeStringAudio(mimeType) {
			...
			clonedLayers := make([]*conkit.VideoLayer, 0, len(req.Layers))
	...
	if p.getPublishedTrackBySignalCid(req.Cid) != nil || p.getPublishedTrackBySdpCid(req.Cid) != nil || p.pendingTracks[req.Cid] != nil {
		if p.pendingTracks[req.Cid] == nil {
			p.pendingTracks[req.Cid] = &pendingTrackInfo{trackInfos: []*conkit.TrackInfo{ti}, createdAt: time.Now(), queued: true}
		} else {
	...
	p.pendingTracks[req.Cid] = &pendingTrackInfo{trackInfos: []*conkit.TrackInfo{ti}, createdAt: time.Now()}
	p.pubLogger.Debugw("pending track added", "trackID", ti.Sid, "track", logger.Proto(ti), "request", logger.Proto(req))
	return ti
}

func (p *ParticipantImpl) GetPendingTrack(trackID conkit.TrackID) *conkit.TrackInfo {
	p.pendingTracksLock.RLock()
	defer p.pendingTracksLock.RUnlock()

	for _, t := range p.pendingTracks {
		if conkit.TrackID(t.trackInfos[0].Sid) == trackID {
...
func (p *ParticipantImpl) sendTrackPublished(cid string, ti *conkit.TrackInfo) {
	p.pubLogger.Debugw("sending track published", "cid", cid, "trackInfo", logger.Proto(ti))
	_ = p.writeMessage(&conkit.SignalResponse{
		Message: &conkit.SignalResponse_TrackPublished{
			TrackPublished: &conkit.TrackPublishedResponse{
				Cid:   cid,
				Track: ti,
			},
		},
...
func (p *ParticipantImpl) SetTrackMuted(trackID conkit.TrackID, muted bool, fromAdmin bool) *conkit.TrackInfo {
...
func (p *ParticipantImpl) setTrackMuted(trackID conkit.TrackID, muted bool) *conkit.TrackInfo {
	p.dirty.Store(true)
	if p.supervisor != nil {
	...
	var trackInfo *conkit.TrackInfo
	if track != nil {
	...
	for _, pti := range p.pendingTracks {
		for i, ti := range pti.trackInfos {
			if conkit.TrackID(ti.Sid) == trackID {
...
func (p *ParticipantImpl) addMigratedTrack(cid string, ti *conkit.TrackInfo) *MediaTrack {
...
func (p *ParticipantImpl) addMediaTrack(signalCid string, sdpCid string, ti *conkit.TrackInfo) *MediaTrack {
	mt := NewMediaTrack(MediaTrackParams{
		...
		Telemetry:             p.params.Telemetry,
		Logger:                LoggerWithTrack(p.pubLogger, conkit.TrackID(ti.Sid), false),
	...
	if p.supervisor != nil {
		p.supervisor.SetPublishedTrack(conkit.TrackID(ti.Sid), mt)
	}
	p.UpTrackManager.AddPublishedTrack(mt)

	pti := p.pendingTracks[signalCid]
	if pti != nil {
		if p.pendingPublishingTracks[conkit.TrackID(ti.Sid)] != nil {
			p.pubLogger.Infow("unexpected pending publish track", "trackID", ti.Sid)
		}
		p.pendingPublishingTracks[conkit.TrackID(ti.Sid)] = &pendingTrackInfo{
			trackInfos: []*conkit.TrackInfo{pti.trackInfos[0]},
...
func (p *ParticipantImpl) getPendingTrack(clientId string, kind conkit.TrackType, skipQueued bool) (string, *conkit.TrackInfo, bool, time.Time) {
	signalCid := clientId
...
func (p *ParticipantImpl) setTrackID(cid string, info *conkit.TrackInfo) {
	var trackID string
	...
	if trackID == "" {
		...
		if info.Type == conkit.TrackType_VIDEO {
			trackPrefix += "V"
		} else if info.Type == conkit.TrackType_AUDIO {
			trackPrefix += "A"
		}
		switch info.Source {
...
func (p *ParticipantImpl) getAndDeleteForwarderState(trackID conkit.TrackID) *conkit.RTPForwarderState {
	p.lock.Lock()
...
func (p *ParticipantImpl) CacheDownTrack(trackID conkit.TrackID, rtpTransceiver *webrtc.RTPTransceiver, downTrack sfu.DownTrackState) {
	p.lock.Lock()
...
func (p *ParticipantImpl) GetCachedDownTrack(trackID conkit.TrackID) (*webrtc.RTPTransceiver, sfu.DownTrackState) {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
func (p *ParticipantImpl) onPublicationError(trackID conkit.TrackID) {
...
func (p *ParticipantImpl) onSubscriptionError(trackID conkit.TrackID, fatal bool, err error) {
	signalErr := conkit.SubscriptionError_SE_UNKNOWN
	switch {
	case errors.Is(err, webrtc.ErrUnsupportedCodec):
		signalErr = conkit.SubscriptionError_SE_CODEC_UNSUPPORTED
	case errors.Is(err, ErrTrackNotFound):
		signalErr = conkit.SubscriptionError_SE_TRACK_NOTFOUND
	}

	_ = p.writeMessage(&conkit.SignalResponse{
		Message: &conkit.SignalResponse_SubscriptionResponse{
			SubscriptionResponse: &conkit.SubscriptionResponse{
				TrackSid: string(trackID),
				Err:      signalErr,
			},
		},
...
func (p *ParticipantImpl) UpdateSubscribedQuality(nodeID conkit.NodeID, trackID conkit.TrackID, maxQualities []types.SubscribedCodecQuality) error {
...
func (p *ParticipantImpl) UpdateMediaLoss(nodeID conkit.NodeID, trackID conkit.TrackID, fractionalLoss uint32) error {
	track := p.GetPublishedTrack(trackID)
	if track == nil {
		p.pubLogger.Debugw("could not find track", "trackID", trackID)
		return errors.New("could not find published track")
...
func (p *ParticipantImpl) GetPlayoutDelayConfig() *conkit.PlayoutDelay {
...
func (p *ParticipantImpl) SendDataPacket(kind conkit.DataPacket_Kind, encoded []byte) error {
	if p.State() != conkit.ParticipantInfo_ACTIVE {
...
func (p *ParticipantImpl) setupEnabledCodecs(publishEnabledCodecs []*conkit.Codec, subscribeEnabledCodecs []*conkit.Codec, disabledCodecs *conkit.DisabledCodecs) {
	shouldDisable := func(c *conkit.Codec, disabled []*conkit.Codec) bool {
	...
	publishCodecs := make([]*conkit.Codec, 0, len(publishEnabledCodecs))
	...
	subscribeCodecs := make([]*conkit.Codec, 0, len(subscribeEnabledCodecs))
...
func (p *ParticipantImpl) GetEnabledPublishCodecs() []*conkit.Codec {
	codecs := make([]*conkit.Codec, 0, len(p.enabledPublishCodecs))
...
func (p *ParticipantImpl) UpdateAudioTrack(update *conkit.UpdateLocalAudioTrack) error {
		...
		for _, ti := range pti.trackInfos {
			if ti.Sid == update.TrackSid {
				...
				for _, feature := range update.Features {
					switch feature {
					case conkit.AudioTrackFeature_TF_STEREO:
						ti.Stereo = true
					case conkit.AudioTrackFeature_TF_NO_DTX:
						ti.DisableDtx = true
					}
				}
...
func (p *ParticipantImpl) UpdateVideoTrack(update *conkit.UpdateLocalVideoTrack) error {
...
func (p *ParticipantImpl) HandleMetrics(senderParticipantID conkit.ParticipantID, metrics *conkit.MetricsBatch) error {
	if p.State() != conkit.ParticipantInfo_ACTIVE {
		return ErrDataChannelUnavailable
	}
