...
type Room struct {
	...
	protoRoom  *conkit.Room
	internal   *conkit.RoomInternal
	protoProxy *utils.ProtoProxy[*conkit.Room]
	...
	participants              map[conkit.ParticipantIdentity]types.LocalParticipant
	participantOpts           map[conkit.ParticipantIdentity]*ParticipantOptions
	participantRequestSources map[conkit.ParticipantIdentity]routing.MessageSource
	hasPublished              map[conkit.ParticipantIdentity]bool
	agentParticpants          map[conkit.ParticipantIdentity]*agentJob
	bufferFactory             *buffer.FactoryOfBufferFactory

	// batch update participant info for non-publishers
	batchedUpdates   map[conkit.ParticipantIdentity]*participantUpdate
	...
	disconnectSignalOnResumeParticipants           map[conkit.ParticipantIdentity]time.Time
	disconnectSignalOnResumeNoMessagesParticipants map[conkit.ParticipantIdentity]*disconnectSignalOnResumeNoMessages
...
func newAgentDispatch(ad *conkit.AgentDispatch) *agentDispatch {
...
func newAgentJob(j *conkit.Job) *agentJob {
	return &agentJob{
		Job:  j,
...
func NewRoom(
	...
	r := &Room{
		...
		participants:                         make(map[conkit.ParticipantIdentity]types.LocalParticipant),
		participantOpts:                      make(map[conkit.ParticipantIdentity]*ParticipantOptions),
		participantRequestSources:            make(map[conkit.ParticipantIdentity]routing.MessageSource),
		hasPublished:                         make(map[conkit.ParticipantIdentity]bool),
		agentParticpants:                     make(map[conkit.ParticipantIdentity]*agentJob),
		...
		batchedUpdates:                       make(map[conkit.ParticipantIdentity]*participantUpdate),
		closed:                               make(chan struct{}),
		trailer:                              []byte(utils.RandomSecret()),
		disconnectSignalOnResumeParticipants: make(map[conkit.ParticipantIdentity]time.Time),
		disconnectSignalOnResumeNoMessagesParticipants: make(map[conkit.ParticipantIdentity]*disconnectSignalOnResumeNoMessages),
	...
	r.protoProxy = utils.NewProtoProxy[*conkit.Room](roomUpdateInterval, r.updateProto)
...
func (r *Room) ToProto() *conkit.Room {
	return r.protoProxy.Get()
}

func (r *Room) Name() conkit.RoomName {
	return conkit.RoomName(r.protoRoom.Name)
}

func (r *Room) ID() conkit.RoomID {
	return conkit.RoomID(r.protoRoom.Sid)
}
...
func (r *Room) GetParticipant(identity conkit.ParticipantIdentity) types.LocalParticipant {
	r.lock.RLock()
	defer r.lock.RUnlock()
	return r.participants[identity]
}

func (r *Room) GetParticipantByID(participantID conkit.ParticipantID) types.LocalParticipant {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
func (r *Room) GetActiveSpeakers() []*conkit.SpeakerInfo {
	participants := r.GetParticipants()
	speakers := make([]*conkit.SpeakerInfo, 0, len(participants))
	for _, p := range participants {
		level, active := p.GetAudioLevel()
		if !active {
			continue
		}
		speakers = append(speakers, &conkit.SpeakerInfo{
			Sid:    string(p.ID()),
			Level:  float32(level),
			Active: active,
		})
...
func (r *Room) Internal() *conkit.RoomInternal {
	return r.internal
}
...
func (r *Room) Join(participant types.LocalParticipant, requestSource routing.MessageSource, opts *ParticipantOptions, iceServers []*conkit.ICEServer) error {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	participant.OnStateChange(func(p types.LocalParticipant, state conkit.ParticipantInfo_State) {
		...
		if state == conkit.ParticipantInfo_ACTIVE {
			...
			meta := &conkit.AnalyticsClientMeta{
		...
		} else if state == conkit.ParticipantInfo_DISCONNECTED {
	...
	participant.OnSubscribeStatusChanged(func(publisherID conkit.ParticipantID, subscribed bool) {
		if subscribed {
			pub := r.GetParticipantByID(publisherID)
			if pub != nil && pub.State() == conkit.ParticipantInfo_ACTIVE {
				...
				if active {
					_ = participant.SendSpeakerUpdate([]*conkit.SpeakerInfo{
						{
							Sid:    string(pub.ID()),
							Level:  float32(level),
							Active: active,
				...
				if cq := pub.GetConnectionQuality(); cq != nil {
					update := &conkit.ConnectionQualityUpdate{}
	...
	r.launchTargetAgents(maps.Values(r.agentDispatches), participant, conkit.JobType_JT_PARTICIPANT)
...
func (r *Room) ReplaceParticipantRequestSource(identity conkit.ParticipantIdentity, reqSource routing.MessageSource) {
	r.lock.Lock()
...
func (r *Room) GetParticipantRequestSource(identity conkit.ParticipantIdentity) routing.MessageSource {
	r.lock.RLock()
	defer r.lock.RUnlock()
...
func (r *Room) ResumeParticipant(
	...
	iceConfig *conkit.ICEConfig,
	iceServers []*conkit.ICEServer,
	...
	if err := p.HandleReconnectAndSendResponse(reason, &conkit.ReconnectResponse{
...
func (r *Room) RemoveParticipant(identity conkit.ParticipantIdentity, pID conkit.ParticipantID, reason types.ParticipantCloseReason) {
	r.lock.Lock()
...
func (r *Room) UpdateSubscriptions(
	...
	participantTracks []*conkit.ParticipantTracks,
	subscribe bool,
) {
	...
	for _, pt := range participantTracks {
		for _, trackID := range conkit.StringsAsIDs[conkit.TrackID](pt.TrackSids) {
			if subscribe {
				participant.SubscribeToTrack(trackID)
			} else {
				participant.UnsubscribeFromTrack(trackID)
...
func (r *Room) SyncState(participant types.LocalParticipant, state *conkit.SyncState) error {
	...
	for _, pubTrack := range pubTracks {
		...
		for _, existingPubTrack := range existingPubTracks {
			if existingPubTrack.ID() == conkit.TrackID(ti.Sid) {
				found = true
				break
			}
		}
		if !found {
			...
			found = participant.GetPendingTrack(conkit.TrackID(ti.Sid)) != nil
		}
		if !found {
	...
	for _, trackSid := range state.TrackSidsDisabled {
		participant.UpdateSubscribedTrackSettings(conkit.TrackID(trackSid), &conkit.UpdateTrackSettings{Disabled: true})
	}

	r.UpdateSubscriptions(
		participant,
		conkit.StringsAsIDs[conkit.TrackID](state.Subscription.TrackSids),
...
func (r *Room) UpdateSubscriptionPermission(participant types.LocalParticipant, subscriptionPermission *conkit.SubscriptionPermission) error {
...
func (r *Room) ResolveMediaTrackForSubscriber(sub types.LocalParticipant, trackID conkit.TrackID) types.MediaResolverResult {
...
func (r *Room) SendDataPacket(dp *conkit.DataPacket, kind conkit.DataPacket_Kind) {
...
func (r *Room) GetAgentDispatches(dispatchID string) ([]*conkit.AgentDispatch, error) {
	r.lock.RLock()
	defer r.lock.RUnlock()

	var ret []*conkit.AgentDispatch
...
func (r *Room) AddAgentDispatch(dispatch *conkit.AgentDispatch) (*conkit.AgentDispatch, error) {
	...
	for _, p := range r.participants {
		if p.IsPublisher() {
			r.launchTargetAgents([]*agentDispatch{ad}, p, conkit.JobType_JT_PUBLISHER)
		}
		r.launchTargetAgents([]*agentDispatch{ad}, p, conkit.JobType_JT_PARTICIPANT)
	}
	r.lock.RUnlock()
...
func (r *Room) DeleteAgentDispatch(dispatchID string) (*conkit.AgentDispatch, error) {
	r.lock.Lock()
	...
	go func() {
		...
		for _, j := range jobs {
			...
			if state.ParticipantIdentity != "" {
				r.lock.RLock()
				agentJob := r.agentParticpants[conkit.ParticipantIdentity(state.ParticipantIdentity)]
				p := r.participants[conkit.ParticipantIdentity(state.ParticipantIdentity)]
				r.lock.RUnlock()
...
func (r *Room) SimulateScenario(participant types.LocalParticipant, simulateScenario *conkit.SimulateScenario) error {
	switch scenario := simulateScenario.Scenario.(type) {
	case *conkit.SimulateScenario_SpeakerUpdate:
		...
		go func() {
			...
			r.sendSpeakerChanges([]*conkit.SpeakerInfo{{
	...
	case *conkit.SimulateScenario_Migration:
	...
	case *conkit.SimulateScenario_NodeFailure:
	...
	case *conkit.SimulateScenario_ServerLeave:
	...
	case *conkit.SimulateScenario_SwitchCandidateProtocol:
		r.Logger.Infow("simulating switch candidate protocol", "participant", participant.Identity())
		participant.ICERestart(&conkit.ICEConfig{
			PreferenceSubscriber: conkit.ICECandidateType(scenario.SwitchCandidateProtocol),
			PreferencePublisher:  conkit.ICECandidateType(scenario.SwitchCandidateProtocol),
		})
	case *conkit.SimulateScenario_SubscriberBandwidth:
	...
	case *conkit.SimulateScenario_DisconnectSignalOnResume:
		...
		r.simulationLock.Unlock()
	case *conkit.SimulateScenario_DisconnectSignalOnResumeNoMessages:
...
func (r *Room) getOtherParticipantInfo(identity conkit.ParticipantIdentity) []*conkit.ParticipantInfo {
	participants := r.GetParticipants()
	pi := make([]*conkit.ParticipantInfo, 0, len(participants))
	for _, p := range participants {
...
func (r *Room) createJoinResponseLocked(participant types.LocalParticipant, iceServers []*conkit.ICEServer) *conkit.JoinResponse {
	// gather other participants and send join response
	otherParticipants := make([]*conkit.ParticipantInfo, 0, len(r.participants))
	for _, p := range r.participants {
	...
	hasICEFallback := iceConfig.GetPreferencePublisher() != conkit.ICECandidateType_ICT_NONE || iceConfig.GetPreferenceSubscriber() != conkit.ICECandidateType_ICT_NONE
	return &conkit.JoinResponse{
		Room:              r.ToProto(),
...
func (r *Room) onTrackPublished(participant types.LocalParticipant, track types.MediaTrack) {
	...
	for _, existingParticipant := range r.participants {
		...
		if existingParticipant.State() != conkit.ParticipantInfo_ACTIVE {
	...
	if !hasPublished {
		r.lock.RLock()
		r.launchTargetAgents(maps.Values(r.agentDispatches), participant, conkit.JobType_JT_PUBLISHER)
		r.lock.RUnlock()
	...
	if participant.Kind() != conkit.ParticipantInfo_EGRESS && r.internal != nil && r.internal.TrackEgress != nil {
		go func() {
...
func (r *Room) onDataPacket(source types.LocalParticipant, kind conkit.DataPacket_Kind, dp *conkit.DataPacket) {
	BroadcastDataPacketForRoom(r, source, kind, dp, r.Logger)
}

func (r *Room) onMetrics(source types.Participant, dp *conkit.DataPacket) {
...
func (r *Room) broadcastParticipantState(p types.LocalParticipant, opts broadcastOptions) {
	...
	if p.Hidden() {
		if !opts.skipSource {
			...
			err := p.SendParticipantUpdate([]*conkit.ParticipantInfo{pi})
			if err != nil {
...
func (r *Room) sendParticipantUpdates(updates []*participantUpdate) {
	...
	var filteredUpdates []*conkit.ParticipantInfo
	...
	var fullUpdates []*conkit.ParticipantInfo
	for _, update := range updates {
...
func (r *Room) sendSpeakerChanges(speakers []*conkit.SpeakerInfo) {
	for _, p := range r.GetParticipants() {
	...
	if existing != nil {
		if pi.Sid == existing.pi.Sid {
			...
			if CompareParticipant(existing.pi, pi) < 0 {
				...
				shouldSend = true
				existing.pi.State = conkit.ParticipantInfo_DISCONNECTED
...
func (r *Room) updateProto() *conkit.Room {
	r.lock.RLock()
...
func (r *Room) changeUpdateWorker() {
	...
	for !r.IsClosed() {
		...
		case <-subTicker.C:
			...
			r.batchedUpdates = make(map[conkit.ParticipantIdentity]*participantUpdate)
...
func (r *Room) audioUpdateWorker() {
	lastActiveMap := make(map[conkit.ParticipantID]*conkit.SpeakerInfo)
	for {
		if r.IsClosed() {
			return
		}
		...
		changedSpeakers := make([]*conkit.SpeakerInfo, 0, len(activeSpeakers))
		nextActiveMap := make(map[conkit.ParticipantID]*conkit.SpeakerInfo, len(activeSpeakers))
		for _, speaker := range activeSpeakers {
			prev := lastActiveMap[conkit.ParticipantID(speaker.Sid)]
			if prev == nil || prev.Level != speaker.Level {
				changedSpeakers = append(changedSpeakers, speaker)
			}
			nextActiveMap[conkit.ParticipantID(speaker.Sid)] = speaker
		}
...
func (r *Room) connectionQualityWorker() {
	...
	prevConnectionInfos := make(map[conkit.ParticipantID]*conkit.ConnectionQualityInfo)
	// send updates to only users that are subscribed to each other
	for !r.IsClosed() {
		<-ticker.C

		participants := r.GetParticipants()
		nowConnectionInfos := make(map[conkit.ParticipantID]*conkit.ConnectionQualityInfo, len(participants))
		...
		maybeAddToUpdate := func(pID conkit.ParticipantID, update *conkit.ConnectionQualityUpdate) {
		...
		for _, op := range participants {
			if !op.ProtocolVersion().SupportsConnectionQuality() || op.State() != conkit.ParticipantInfo_ACTIVE {
				continue
			}
			update := &conkit.ConnectionQualityUpdate{}
...
func (r *Room) launchTargetAgents(ads []*agentDispatch, p types.Participant, jobType conkit.JobType) {
...
func (r *Room) handleNewJobs(ad *conkit.AgentDispatch, inc *sutils.IncrementalDispatcher[*conkit.Job]) {
	inc.ForEach(func(job *conkit.Job) {
		...
		if job.State != nil && job.State.ParticipantIdentity != "" {
			r.agentParticpants[conkit.ParticipantIdentity(job.State.ParticipantIdentity)] = newAgentJob(job)
		}
		r.lock.Unlock()
	})
}
...
func (r *Room) createAgentDispatch(dispatch *conkit.AgentDispatch) (*agentDispatch, error) {
	dispatch.State = &conkit.AgentDispatchState{
...
func (r *Room) createAgentDispatchFromParams(agentName string, metadata string) (*agentDispatch, error) {
	return r.createAgentDispatch(&conkit.AgentDispatch{
...
func (r *Room) createAgentDispatchesFromRoomAgent() {
	...
	if len(roomDisp) == 0 {
		...
		roomDisp = []*conkit.RoomAgentDispatch{{}}
	}
...
func (r *Room) IsDataMessageUserPacketDuplicate(up *conkit.UserPacket) bool {
	return r.userPacketDeduper.IsDuplicate(up)
}

// ------------------------------------------------------------

func BroadcastDataPacketForRoom(r types.Room, source types.LocalParticipant, kind conkit.DataPacket_Kind, dp *conkit.DataPacket, logger logger.Logger) {
...
func BroadcastMetricsForRoom(r types.Room, source types.Participant, dp *conkit.DataPacket, logger logger.Logger) {
...
func CompareParticipant(pi1 *conkit.ParticipantInfo, pi2 *conkit.ParticipantInfo) int {
