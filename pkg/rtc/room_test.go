...
import (
	...
	"github.com/a4to/conkit-server/pkg/sfu/audio"
...
var iceServersForRoom = []*conkit.ICEServer{{Urls: []string{"stun:stun.l.google.com:19302"}}}
...
func TestRoomJoin(t *testing.T) {
	t.Run("joining returns existing participant data", func(t *testing.T) {
		...
		require.Equal(t, conkit.RoomID(res.Room.Sid), rm.ID())
	...
	t.Run("subscribe to existing channels upon join", func(t *testing.T) {
		...
		stateChangeCB(p, conkit.ParticipantInfo_ACTIVE)
	...
	t.Run("participant state change is broadcasted to others", func(t *testing.T) {
		...
		disconnectedParticipant.StateReturns(conkit.ParticipantInfo_DISCONNECTED)
...
func TestParticipantUpdate(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			callCounts := make(map[conkit.ParticipantID]int)
...
func TestPushAndDequeueUpdates(t *testing.T) {
	identity := "test_user"
	publisher1v1 := &conkit.ParticipantInfo{
		Identity:    identity,
		Sid:         "1",
		IsPublisher: true,
		Version:     1,
	...
	publisher1v2 := &conkit.ParticipantInfo{
		Identity:    identity,
		Sid:         "1",
		IsPublisher: true,
		Version:     2,
	...
	publisher2 := &conkit.ParticipantInfo{
		Identity:    identity,
		Sid:         "2",
		IsPublisher: true,
		Version:     1,
	...
	subscriber1v1 := &conkit.ParticipantInfo{
		Identity: identity,
		Sid:      "1",
		Version:  1,
		JoinedAt: 0,
	}
	subscriber1v2 := &conkit.ParticipantInfo{
		Identity: identity,
		Sid:      "1",
		Version:  2,
		JoinedAt: 1,
	...
	requirePIEquals := func(t *testing.T, a, b *conkit.ParticipantInfo) {
	...
	testCases := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, rm *Room, _ []*participantUpdate) {
				queued := rm.batchedUpdates[conkit.ParticipantIdentity(identity)]
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			rm := newRoomWithParticipants(t, testRoomOpts{num: 1})
			if tc.existing != nil {
				rm.batchedUpdates[conkit.ParticipantIdentity(tc.existing.pi.Identity)] = tc.existing
			}
...
func TestNewTrack(t *testing.T) {
	t.Run("new track should be added to ready participants", func(t *testing.T) {
		...
		p0.StateReturns(conkit.ParticipantInfo_JOINED)
		p1 := participants[1].(*typesfakes.FakeLocalParticipant)
		p1.StateReturns(conkit.ParticipantInfo_ACTIVE)

		pub := participants[2].(*typesfakes.FakeLocalParticipant)

		// pub adds track
		track := NewMockTrack(conkit.TrackType_VIDEO, "webcam")
...
func TestActiveSpeakers(t *testing.T) {
	t.Parallel()
	getActiveSpeakerUpdates := func(p *typesfakes.FakeLocalParticipant) [][]*conkit.SpeakerInfo {
		var updates [][]*conkit.SpeakerInfo
...
func TestDataChannel(t *testing.T) {
	...
	setSource := func(mode int, dp *conkit.DataPacket, p types.LocalParticipant) {
		switch mode {
		case curAPI:
	...
	setDest := func(mode int, dp *conkit.DataPacket, p types.LocalParticipant) {
		switch mode {
		case curAPI:
	...
	t.Run("participants should receive data", func(t *testing.T) {
		for _, mode := range modes {
			...
			t.Run(modeNames[mode], func(t *testing.T) {
				...
				packet := &conkit.DataPacket{
					Kind: conkit.DataPacket_RELIABLE,
...
func TestHiddenParticipants(t *testing.T) {
	...
	t.Run("hidden participant subscribes to tracks", func(t *testing.T) {
		...
		require.NotNil(t, stateChangeCB)
		stateChangeCB(hidden, conkit.ParticipantInfo_ACTIVE)
...
func newRoomWithParticipants(t *testing.T, opts testRoomOpts) *Room {
	...
	for i := 0; i < opts.num+opts.numHidden; i++ {
		identity := conkit.ParticipantIdentity(fmt.Sprintf("p%d", i))
		...
		require.NoError(t, err)
		participant.StateReturns(conkit.ParticipantInfo_ACTIVE)
