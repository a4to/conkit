...
type RoomTrackManager struct {
	...
	tracks          map[conkit.TrackID]*TrackInfo
}
...
func NewRoomTrackManager() *RoomTrackManager {
	return &RoomTrackManager{
		tracks:          make(map[conkit.TrackID]*TrackInfo),
		changedNotifier: utils.NewChangeNotifierManager(),
		removedNotifier: utils.NewChangeNotifierManager(),
	}
}

func (r *RoomTrackManager) AddTrack(track types.MediaTrack, publisherIdentity conkit.ParticipantIdentity, publisherID conkit.ParticipantID) {
	r.lock.Lock()
...
func (r *RoomTrackManager) GetTrackInfo(trackID conkit.TrackID) *TrackInfo {
	r.lock.RLock()
...
func (r *RoomTrackManager) NotifyTrackChanged(trackID conkit.TrackID) {
...
func (r *RoomTrackManager) GetOrCreateTrackChangeNotifier(trackID conkit.TrackID) *utils.ChangeNotifier {
	return r.changedNotifier.GetOrCreateNotifier(string(trackID))
}

func (r *RoomTrackManager) GetOrCreateTrackRemoveNotifier(trackID conkit.TrackID) *utils.ChangeNotifier {
