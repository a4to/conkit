...
const (
	trackIDForReconcileSubscriptions = conkit.TrackID("subscriptions_reconcile")
)
...
type SubscriptionManagerParams struct {
	...
	OnSubscriptionError func(trackID conkit.TrackID, fatal bool, err error)
...
type SubscriptionManager struct {
	...
	lock                sync.RWMutex
	subscriptions       map[conkit.TrackID]*trackSubscription
	pendingUnsubscribes atomic.Int32

	subscribedVideoCount, subscribedAudioCount atomic.Int32

	subscribedTo map[conkit.ParticipantID]map[conkit.TrackID]struct{}
	reconcileCh  chan conkit.TrackID
	closeCh      chan struct{}
	doneCh       chan struct{}

	onSubscribeStatusChanged func(publisherID conkit.ParticipantID, subscribed bool)
}

func NewSubscriptionManager(params SubscriptionManagerParams) *SubscriptionManager {
	m := &SubscriptionManager{
		params:        params,
		subscriptions: make(map[conkit.TrackID]*trackSubscription),
		subscribedTo:  make(map[conkit.ParticipantID]map[conkit.TrackID]struct{}),
		reconcileCh:   make(chan conkit.TrackID, 50),
...
func (m *SubscriptionManager) SubscribeToTrack(trackID conkit.TrackID) {
...
func (m *SubscriptionManager) UnsubscribeFromTrack(trackID conkit.TrackID) {
	if m.params.UseOneShotSignallingMode {
...
func (m *SubscriptionManager) IsTrackNameSubscribed(publisherIdentity conkit.ParticipantIdentity, trackName string) bool {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) StopAndGetSubscribedTracksForwarderState() map[conkit.TrackID]*conkit.RTPForwarderState {
	m.lock.RLock()
	defer m.lock.RUnlock()

	states := make(map[conkit.TrackID]*conkit.RTPForwarderState, len(m.subscriptions))
...
func (m *SubscriptionManager) GetSubscribedParticipants() []conkit.ParticipantID {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) IsSubscribedTo(participantID conkit.ParticipantID) bool {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) UpdateSubscribedTrackSettings(trackID conkit.TrackID, settings *conkit.UpdateTrackSettings) {
	m.lock.Lock()
...
func (m *SubscriptionManager) OnSubscribeStatusChanged(fn func(publisherID conkit.ParticipantID, subscribed bool)) {
	m.lock.Lock()
...
func (m *SubscriptionManager) setDesired(trackID conkit.TrackID, desired bool) (*trackSubscription, bool) {
	m.lock.RLock()
	defer m.lock.RUnlock()
...
func (m *SubscriptionManager) queueReconcile(trackID conkit.TrackID) {
	select {
...
func (m *SubscriptionManager) hasCapacityForSubscription(kind conkit.TrackType) bool {
	switch kind {
	case conkit.TrackType_VIDEO:
		if m.params.SubscriptionLimitVideo > 0 && m.subscribedVideoCount.Load() >= m.params.SubscriptionLimitVideo {
			return false
		}

	case conkit.TrackType_AUDIO:
...
func (m *SubscriptionManager) subscribe(s *trackSubscription) error {
	...
	if pTracks == nil {
		pTracks = make(map[conkit.TrackID]struct{})
...
func (m *SubscriptionManager) subscribeSynchronous(trackID conkit.TrackID) error {
...
func (m *SubscriptionManager) unsubscribeSynchronous(trackID conkit.TrackID) error {
	m.lock.Lock()
...
func (m *SubscriptionManager) handleSourceTrackRemoved(trackID conkit.TrackID) {
	m.lock.Lock()
	sub := m.subscriptions[trackID]
	m.lock.Unlock()
...
func (m *SubscriptionManager) handleSubscribedTrackClose(s *trackSubscription, isExpectedToResume bool) {
	...
	if wasBound {
		m.params.Telemetry.TrackUnsubscribed(
			context.Background(),
			m.params.Participant.ID(),
			&conkit.TrackInfo{Sid: string(s.trackID), Type: subTrack.MediaTrack().Kind()},
			!isExpectedToResume,
		)
...
type trackSubscription struct {
	...
	settings                 *conkit.UpdateTrackSettings
	...
	bound                    bool
	kind                     atomic.Pointer[conkit.TrackType]
...
func newTrackSubscription(subscriberID conkit.ParticipantID, trackID conkit.TrackID, l logger.Logger) *trackSubscription {
	s := &trackSubscription{
...
func (s *trackSubscription) setPublisher(publisherIdentity conkit.ParticipantIdentity, publisherID conkit.ParticipantID) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *trackSubscription) getPublisherID() conkit.ParticipantID {
	s.lock.RLock()
...
func (s *trackSubscription) trySetKind(kind conkit.TrackType) {
	s.kind.CompareAndSwap(nil, &kind)
}

func (s *trackSubscription) getKind() (conkit.TrackType, bool) {
	kind := s.kind.Load()
	if kind == nil {
		return conkit.TrackType_AUDIO, false
	}
	return *kind, true
}
...
func (s *trackSubscription) setSettings(settings *conkit.UpdateTrackSettings) {
	s.lock.Lock()
	s.settings = settings
...
func (s *trackSubscription) maybeRecordError(ts telemetry.TelemetryService, pID conkit.ParticipantID, err error, isUserError bool) {
...
func (s *trackSubscription) maybeRecordSuccess(ts telemetry.TelemetryService, pID conkit.ParticipantID) {
	subTrack := s.getSubscribedTrack()
	if subTrack == nil {
		return
	}
