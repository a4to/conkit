...
import (
	...
	"github.com/a4to/conkit-server/pkg/sfu/bwe"
	"github.com/a4to/conkit-server/pkg/sfu/bwe/remotebwe"
	"github.com/a4to/conkit-server/pkg/sfu/bwe/sendsidebwe"
	"github.com/a4to/conkit-server/pkg/sfu/datachannel"
	sfuinterceptor "github.com/a4to/conkit-server/pkg/sfu/interceptor"
	...
	sfuutils "github.com/a4to/conkit-server/pkg/sfu/utils"
	"github.com/a4to/conkit-server/pkg/telemetry/prometheus"
	"github.com/a4to/conkit-server/pkg/utils"
	lkinterceptor "github.com/a4to/mediatransportutil/pkg/interceptor"
	lktwcc "github.com/a4to/mediatransportutil/pkg/twcc"
	"github.com/a4to/protocol/conkit"
	"github.com/a4to/protocol/logger"
	"github.com/a4to/protocol/logger/pionlogger"
	lksdp "github.com/a4to/protocol/sdp"
)
...
type TransportParams struct {
	...
	EnabledCodecs                []*conkit.Codec
	Logger                       logger.Logger
	Transport                    conkit.SignalTarget
...
func (t *PCTransport) onDataChannel(dc *webrtc.DataChannel) {
	dc.OnOpen(func() {
		...
		var kind conkit.DataPacket_Kind
		switch dc.Label() {
		case ReliableDataChannel:
			kind = conkit.DataPacket_RELIABLE

		case LossyDataChannel:
			kind = conkit.DataPacket_LOSSY
		...
		switch kind {
		case conkit.DataPacket_RELIABLE:
			t.lock.Lock()
			if t.reliableDC != nil {
				t.reliableDC.Close()
			}
...
func (t *PCTransport) SendDataPacket(kind conkit.DataPacket_Kind, encoded []byte) error {
	var dc *datachannel.DataChannelWriter[*webrtc.DataChannel]
	t.lock.RLock()
	if kind == conkit.DataPacket_RELIABLE {
		dc = t.reliableDC
	} else {
		dc = t.lossyDC
	}
