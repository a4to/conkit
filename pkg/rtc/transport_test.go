...
func handleICEExchange(t *testing.T, a, b *PCTransport, ah, bh *transportfakes.FakeHandler) {
	ah.OnICECandidateCalls(func(candidate *webrtc.ICECandidate, target conkit.SignalTarget) error {
		if candidate == nil {
			return nil
		}
	...
	bh.OnICECandidateCalls(func(candidate *webrtc.ICECandidate, target conkit.SignalTarget) error {
		if candidate == nil {
			return nil
		}
...
func TestConfigureAudioTransceiver(t *testing.T) {
	for _, testcase := range []struct {
		...
		t.Run(fmt.Sprintf("nack=%v,stereo=%v", testcase.nack, testcase.stereo), func(t *testing.T) {
			...
			registerCodecs(&me, []*conkit.Codec{{Mime: mime.MimeTypeOpus.String()}}, RTCPFeedbackConfig{Audio: []webrtc.RTCPFeedback{{Type: webrtc.TypeRTCPFBNACK}}}, false)
