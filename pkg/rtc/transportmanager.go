...
import (
	...
	"github.com/a4to/conkit-server/pkg/sfu/datachannel"
...
type TransportManager struct {
	...
	pendingDataChannelsPublisher []*conkit.DataChannelInfo
	...
	onICEConfigChanged func(iceConfig *conkit.ICEConfig)
...
func NewTransportManager(params TransportManagerParams) (*TransportManager, error) {
	...
	lgr := LoggerWithPCTarget(params.Logger, conkit.SignalTarget_PUBLISHER)
	publisher, err := NewPCTransport(TransportParams{
		...
		Transport:                    conkit.SignalTarget_PUBLISHER,
	...
	lgr = LoggerWithPCTarget(params.Logger, conkit.SignalTarget_SUBSCRIBER)
	subscriber, err := NewPCTransport(TransportParams{
		...
		DatachannelSlowThreshold: params.DatachannelSlowThreshold,
		Transport:                conkit.SignalTarget_SUBSCRIBER,
...
func (t *TransportManager) SendDataPacket(kind conkit.DataPacket_Kind, encoded []byte) error {
...
func (t *TransportManager) createDataChannelsForSubscriber(pendingDataChannels []*conkit.DataChannelInfo) error {
	var (
...
func (t *TransportManager) AddICECandidate(candidate webrtc.ICECandidateInit, target conkit.SignalTarget) {
	switch target {
	case conkit.SignalTarget_PUBLISHER:
		t.publisher.AddICECandidate(candidate)
	case conkit.SignalTarget_SUBSCRIBER:
...
func (t *TransportManager) HandleClientReconnect(reason conkit.ReconnectReason) {
	var (
		isShort              bool
	...
	switch reason {
	case conkit.ReconnectReason_RR_PUBLISHER_FAILED:
		resetShortConnection = true
		isShort, duration = t.publisher.IsShortConnection(time.Now())

	case conkit.ReconnectReason_RR_SUBSCRIBER_FAILED:
		resetShortConnection = true
...
func (t *TransportManager) ICERestart(iceConfig *conkit.ICEConfig) error {
	t.SetICEConfig(iceConfig)

	return t.subscriber.ICERestart()
}

func (t *TransportManager) OnICEConfigChanged(f func(iceConfig *conkit.ICEConfig)) {
	t.lock.Lock()
	t.onICEConfigChanged = f
	t.lock.Unlock()
}

func (t *TransportManager) SetICEConfig(iceConfig *conkit.ICEConfig) {
	if iceConfig != nil {
		t.configureICE(iceConfig, true)
	}
}

func (t *TransportManager) GetICEConfig() *conkit.ICEConfig {
	t.lock.RLock()
...
func (t *TransportManager) configureICE(iceConfig *conkit.ICEConfig, reset bool) {
	t.lock.Lock()
	...
	if iceConfig.PreferenceSubscriber != conkit.ICECandidateType_ICT_NONE {
		t.mediaLossProxy.OnMediaLossUpdate(nil)
	}

	t.publisher.SetPreferTCP(iceConfig.PreferencePublisher == conkit.ICECandidateType_ICT_TCP)
	t.subscriber.SetPreferTCP(iceConfig.PreferenceSubscriber == conkit.ICECandidateType_ICT_TCP)
...
func (t *TransportManager) handleConnectionFailed(isShortLived bool) {
	...
	//
	getNext := func(ic *conkit.ICEConfig) conkit.ICECandidateType {
		if ic.PreferenceSubscriber == conkit.ICECandidateType_ICT_NONE && t.params.ClientInfo.SupportsICETCP() && t.canUseICETCP() {
			return conkit.ICECandidateType_ICT_TCP
		} else if ic.PreferenceSubscriber != conkit.ICECandidateType_ICT_TLS && t.params.TURNSEnabled {
			return conkit.ICECandidateType_ICT_TLS
		} else {
			return conkit.ICECandidateType_ICT_NONE
		}
	}

	var preferNext conkit.ICECandidateType
	if isShortLived {
	...
	t.configureICE(&conkit.ICEConfig{
		PreferenceSubscriber: preferNext,
		PreferencePublisher:  preferNext,
	}, false)
}

func (t *TransportManager) SetMigrateInfo(previousOffer, previousAnswer *webrtc.SessionDescription, dataChannels []*conkit.DataChannelInfo) {
	t.lock.Lock()
	t.pendingDataChannelsPublisher = make([]*conkit.DataChannelInfo, 0, len(dataChannels))
	pendingDataChannelsSubscriber := make([]*conkit.DataChannelInfo, 0, len(dataChannels))
	for _, dci := range dataChannels {
		if dci.Target == conkit.SignalTarget_SUBSCRIBER {
