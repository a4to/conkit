...
type UpTrackManager struct {
	...
	publishedTracks        map[conkit.TrackID]types.MediaTrack
	subscriptionPermission *conkit.SubscriptionPermission
	// subscriber permission for published tracks
	subscriberPermissions map[conkit.ParticipantIdentity]*conkit.TrackPermission // subscriberIdentity => *conkit.TrackPermission
...
func NewUpTrackManager(params UpTrackManagerParams) *UpTrackManager {
	return &UpTrackManager{
		params:          params,
		publishedTracks: make(map[conkit.TrackID]types.MediaTrack),
	}
}
...
func (u *UpTrackManager) Close(isExpectedToResume bool) {
	...
	u.publishedTracks = make(map[conkit.TrackID]types.MediaTrack)
	u.lock.Unlock()
...
func (u *UpTrackManager) ToProto() []*conkit.TrackInfo {
	u.lock.RLock()
	defer u.lock.RUnlock()

	var trackInfos []*conkit.TrackInfo
...
func (u *UpTrackManager) SetPublishedTrackMuted(trackID conkit.TrackID, muted bool) types.MediaTrack {
...
func (u *UpTrackManager) GetPublishedTrack(trackID conkit.TrackID) types.MediaTrack {
	u.lock.RLock()
	defer u.lock.RUnlock()
...
func (u *UpTrackManager) UpdateSubscriptionPermission(
	...
	resolverBySid func(participantID conkit.ParticipantID) types.LocalParticipant,
) error {
	u.lock.Lock()
	...
	if err := u.parseSubscriptionPermissionsLocked(subscriptionPermission, func(pID conkit.ParticipantID) types.LocalParticipant {
		u.lock.Unlock()
...
func (u *UpTrackManager) SubscriptionPermission() (*conkit.SubscriptionPermission, utils.TimedVersion) {
	u.lock.RLock()
	defer u.lock.RUnlock()
...
func (u *UpTrackManager) HasPermission(trackID conkit.TrackID, subIdentity conkit.ParticipantIdentity) bool {
	u.lock.RLock()
	defer u.lock.RUnlock()
...
func (u *UpTrackManager) UpdatePublishedAudioTrack(update *conkit.UpdateLocalAudioTrack) types.MediaTrack {
	track := u.GetPublishedTrack(conkit.TrackID(update.TrackSid))
	if track != nil {
...
func (u *UpTrackManager) UpdatePublishedVideoTrack(update *conkit.UpdateLocalVideoTrack) types.MediaTrack {
	track := u.GetPublishedTrack(conkit.TrackID(update.TrackSid))
	if track != nil {
...
func (u *UpTrackManager) getPublishedTrackLocked(trackID conkit.TrackID) types.MediaTrack {
	return u.publishedTracks[trackID]
}

func (u *UpTrackManager) parseSubscriptionPermissionsLocked(
	subscriptionPermission *conkit.SubscriptionPermission,
	resolver func(participantID conkit.ParticipantID) types.LocalParticipant,
) error {
	...
	subscriberPermissions := make(map[conkit.ParticipantIdentity]*conkit.TrackPermission)
	for _, trackPerms := range subscriptionPermission.TrackPermissions {
		subscriberIdentity := conkit.ParticipantIdentity(trackPerms.ParticipantIdentity)
		if subscriberIdentity == "" {
			if trackPerms.ParticipantSid == "" {
				return ErrSubscriptionPermissionNeedsId
			}

			sub := resolver(conkit.ParticipantID(trackPerms.ParticipantSid))
			if sub == nil {
...
func (u *UpTrackManager) hasPermissionLocked(trackID conkit.TrackID, subscriberIdentity conkit.ParticipantIdentity) bool {
...
func (u *UpTrackManager) getAllowedSubscribersLocked(trackID conkit.TrackID) []conkit.ParticipantIdentity {
	if u.subscriberPermissions == nil {
		return nil
	}

	allowed := make([]conkit.ParticipantIdentity, 0)
...
func (u *UpTrackManager) DebugInfo() map[string]interface{} {
	...
	publishedTrackInfo := make(map[conkit.TrackID]interface{})
...
func (u *UpTrackManager) GetAudioLevel() (level float64, active bool) {
	...
	for _, pt := range u.GetPublishedTracks() {
		if pt.Source() == conkit.TrackSource_MICROPHONE {
