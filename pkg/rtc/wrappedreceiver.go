...
func (r *WrappedReceiver) TrackID() conkit.TrackID {
...
func (r *WrappedReceiver) DeleteDownTrack(participantID conkit.ParticipantID) {
	r.lock.Lock()
...
type DummyReceiver struct {
	...
	downTracks         map[conkit.ParticipantID]sfu.TrackSender
	onReadyCallbacks   []func()
...
func NewDummyReceiver(trackID conkit.TrackID, streamId string, codec webrtc.RTPCodecParameters, headerExtensions []webrtc.RTPHeaderExtensionParameter) *DummyReceiver {
	return &DummyReceiver{
		trackID:          trackID,
		streamId:         streamId,
		codec:            codec,
		headerExtensions: headerExtensions,
		downTracks:       make(map[conkit.ParticipantID]sfu.TrackSender),
	}
}
...
func (d *DummyReceiver) Upgrade(receiver sfu.TrackReceiver) {
	...
	d.downTracks = make(map[conkit.ParticipantID]sfu.TrackSender)
...
func (d *DummyReceiver) TrackID() conkit.TrackID {
	return d.trackID
}
...
func (d *DummyReceiver) DeleteDownTrack(subscriberID conkit.ParticipantID) {
	d.downTrackLock.Lock()
	defer d.downTrackLock.Unlock()
...
func (d *DummyReceiver) TrackInfo() *conkit.TrackInfo {
	if r, ok := d.receiver.Load().(sfu.TrackReceiver); ok {
		return r.TrackInfo()
	}
	return nil
...
func (d *DummyReceiver) UpdateTrackInfo(ti *conkit.TrackInfo) {
	if r, ok := d.receiver.Load().(sfu.TrackReceiver); ok {
...
func (d *DummyReceiver) GetTrackStats() *conkit.RTPStats {
	if r, ok := d.receiver.Load().(sfu.TrackReceiver); ok {
...
func (d *DummyRedReceiver) DeleteDownTrack(subscriberID conkit.ParticipantID) {
	d.downTrackLock.Lock()
	defer d.downTrackLock.Unlock()
