...
func HandshakeAgentWorker(c agent.SignalConn, serverInfo *conkit.ServerInfo, protocol agent.WorkerProtocolVersion, l logger.Logger) (r agent.WorkerRegistration, ok bool) {
...
type AgentHandler struct {
	...
	jobToWorker map[conkit.JobID]*agent.Worker
...
func NewAgentHandler(
	...
	return &AgentHandler{
		...
		jobToWorker:      make(map[conkit.JobID]*agent.Worker),
...
func (h *AgentHandler) deregisterJob(jobID conkit.JobID) {
...
func (h *AgentHandler) JobRequest(ctx context.Context, job *conkit.Job) (*rpc.JobRequestResponse, error) {
	...
	for {
		...
		h.mu.Lock()
		h.jobToWorker[conkit.JobID(job.Id)] = selected
		h.mu.Unlock()
...
func (h *AgentHandler) JobRequestAffinity(ctx context.Context, job *conkit.Job) float32 {
	h.mu.Lock()
	defer h.mu.Unlock()
	...
	for _, w := range h.workers {
		...
		if w.Status() == conkit.WorkerStatus_WS_AVAILABLE {
...
func (h *AgentHandler) JobTerminate(ctx context.Context, req *rpc.JobTerminateRequest) (*rpc.JobTerminateResponse, error) {
	h.mu.Lock()
	w := h.jobToWorker[conkit.JobID(req.JobId)]
	h.mu.Unlock()
	...
	state, err := w.TerminateJob(conkit.JobID(req.JobId), req.Reason)
	if err != nil {
		return nil, err
	}
...
func (h *AgentHandler) selectWorkerWeightedByLoad(key workerKey, ignore map[*agent.Worker]struct{}) (*agent.Worker, error) {
	...
	for _, w := range workers {
		if _, ok := ignore[w]; !ok && w.Status() == conkit.WorkerStatus_WS_AVAILABLE {
...
func (w *agentHandlerWorker) HandleUpdateJob(update *conkit.UpdateJobStatus) error {
	...
	if agent.JobStatusIsEnded(update.Status) {
		w.h.mu.Lock()
		w.h.deregisterJob(conkit.JobID(update.JobId))
		w.h.mu.Unlock()
	}
	return nil
}
