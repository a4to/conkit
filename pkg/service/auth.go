...
func EnsureJoinPermission(ctx context.Context) (name conkit.RoomName, err error) {
	...
	if claims.Video.RoomJoin {
		name = conkit.RoomName(claims.Video.Room)
	} else {
		err = ErrPermissionDenied
	}
	return
...
func EnsureAdminPermission(ctx context.Context, room conkit.RoomName) error {
	claims := GetGrants(ctx)
	if claims == nil || claims.Video == nil {
		return ErrPermissionDenied
	}

	if !claims.Video.RoomAdmin || room != conkit.RoomName(claims.Video.Room) {
		return ErrPermissionDenied
	}
