...
type IngressLauncher interface {
	LaunchPullIngress(ctx context.Context, info *conkit.IngressInfo) (*conkit.IngressInfo, error)
}
...
func (s *IngressService) CreateIngress(ctx context.Context, req *conkit.CreateIngressRequest) (*conkit.IngressInfo, error) {
	fields := []interface{}{
	...
	case conkit.IngressInput_RTMP_INPUT:
		url = s.conf.RTMPBaseURL
	case conkit.IngressInput_WHIP_INPUT:
		url = s.conf.WHIPBaseURL
	case conkit.IngressInput_URL_INPUT:
	default:
...
func (s *IngressService) CreateIngressWithUrl(ctx context.Context, urlStr string, req *conkit.CreateIngressRequest) (*conkit.IngressInfo, error) {
	...
	if req.InputType == conkit.IngressInput_URL_INPUT {
		if req.Url == "" {
	...
	switch req.InputType {
	case conkit.IngressInput_RTMP_INPUT,
		conkit.IngressInput_WHIP_INPUT:
		info.Reusable = true
	...
	if req.InputType == conkit.IngressInput_URL_INPUT {
		...
		if retInfo != nil {
			...
			info.State.Status = conkit.IngressState_ENDPOINT_ERROR
...
func (s *IngressService) LaunchPullIngress(ctx context.Context, info *conkit.IngressInfo) (*conkit.IngressInfo, error) {
...
func updateEnableTranscoding(info *conkit.IngressInfo) {
...
func updateInfoUsingRequest(req *conkit.UpdateIngressRequest, info *conkit.IngressInfo) error {
	if req.Name != "" {
		info.Name = req.Name
	}
	if req.RoomName != "" {
...
func (s *IngressService) UpdateIngress(ctx context.Context, req *conkit.UpdateIngressRequest) (*conkit.IngressInfo, error) {
	fields := []interface{}{
	...
	case conkit.IngressState_ENDPOINT_ERROR:
		info.State.Status = conkit.IngressState_ENDPOINT_INACTIVE
	...
	case conkit.IngressState_ENDPOINT_INACTIVE:
		err = updateInfoUsingRequest(req, info)
		if err != nil {
			return nil, err
		}

	case conkit.IngressState_ENDPOINT_BUFFERING,
		conkit.IngressState_ENDPOINT_PUBLISHING:
		err := updateInfoUsingRequest(req, info)
		if err != nil {
			return nil, err
		}
...
func (s *IngressService) ListIngress(ctx context.Context, req *conkit.ListIngressRequest) (*conkit.ListIngressResponse, error) {
	...
	if req.IngressId != "" {
		...
		infos = []*conkit.IngressInfo{info}
	} else {
		infos, err = s.store.ListIngress(ctx, conkit.RoomName(req.RoomName))
		if err != nil {
	...
	return &conkit.ListIngressResponse{Items: infos}, nil
}

func (s *IngressService) DeleteIngress(ctx context.Context, req *conkit.DeleteIngressRequest) (*conkit.IngressInfo, error) {
