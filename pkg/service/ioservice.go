...
func (s *IOInfoService) CreateEgress(ctx context.Context, info *conkit.EgressInfo) (*emptypb.Empty, error) {
...
func (s *IOInfoService) UpdateEgress(ctx context.Context, info *conkit.EgressInfo) (*emptypb.Empty, error) {
	err := s.es.UpdateEgress(ctx, info)

	switch info.Status {
	case conkit.EgressStatus_EGRESS_ACTIVE,
		conkit.EgressStatus_EGRESS_ENDING:
		s.telemetry.EgressUpdated(ctx, info)

	case conkit.EgressStatus_EGRESS_COMPLETE,
		conkit.EgressStatus_EGRESS_FAILED,
		conkit.EgressStatus_EGRESS_ABORTED,
		conkit.EgressStatus_EGRESS_LIMIT_REACHED:
...
func (s *IOInfoService) GetEgress(ctx context.Context, req *rpc.GetEgressRequest) (*conkit.EgressInfo, error) {
...
func (s *IOInfoService) ListEgress(ctx context.Context, req *conkit.ListEgressRequest) (*conkit.ListEgressResponse, error) {
	if req.EgressId != "" {
		info, err := s.es.LoadEgress(ctx, req.EgressId)
		if err != nil {
			logger.Errorw("failed to load egress", err)
		...
		return &conkit.ListEgressResponse{Items: []*conkit.EgressInfo{info}}, nil
	}

	items, err := s.es.ListEgress(ctx, conkit.RoomName(req.RoomName), req.Active)
	if err != nil {
		logger.Errorw("failed to list egress", err)
		return nil, err
	}

	return &conkit.ListEgressResponse{Items: items}, nil
}
