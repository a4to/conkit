...
func (s *IOInfoService) matchSIPTrunk(ctx context.Context, trunkID, calling, called string, srcIP netip.Addr) (*conkit.SIPInboundTrunkInfo, error) {
	if s.ss == nil {
	...
	if trunkID != "" {
		...
		if tr, err := s.ss.LoadSIPInboundTrunk(ctx, trunkID); err == nil {
			tr, err = sip.MatchTrunkIter(iters.Slice([]*conkit.SIPInboundTrunkInfo{tr}), srcIP, calling, called)
			if err == nil {
				return tr, nil
			}
		}
...
func (s *IOInfoService) SelectSIPInboundTrunk(ctx context.Context, called string) iters.Iter[*conkit.SIPInboundTrunkInfo] {
	it := conkit.ListPageIter(s.ss.ListSIPInboundTrunk, &conkit.ListSIPInboundTrunkRequest{
...
func (s *IOInfoService) matchSIPDispatchRule(ctx context.Context, trunk *conkit.SIPInboundTrunkInfo, req *rpc.EvaluateSIPDispatchRulesRequest) (*conkit.SIPDispatchRuleInfo, error) {
	if s.ss == nil {
		return nil, ErrSIPNotConnected
	}
	var trunkID string
...
func (s *IOInfoService) SelectSIPDispatchRule(ctx context.Context, trunkID string) iters.Iter[*conkit.SIPDispatchRuleInfo] {
	var trunkIDs []string
	if trunkID != "" {
		trunkIDs = []string{trunkID}
	}
	it := conkit.ListPageIter(s.ss.ListSIPDispatchRule, &conkit.ListSIPDispatchRuleRequest{
		TrunkIds: trunkIDs,
	})
	return iters.PagesAsIter(ctx, it)
}
