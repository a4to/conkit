...
func TestSIPStoreDispatch(t *testing.T) {
	...
	list, err := rs.ListSIPDispatchRule(ctx, &conkit.ListSIPDispatchRuleRequest{})
	...
	rule := &conkit.SIPDispatchRuleInfo{
		TrunkIds: []string{"trunk"},
		Rule: &conkit.SIPDispatchRule{Rule: &conkit.SIPDispatchRule_DispatchRuleDirect{
			DispatchRuleDirect: &conkit.SIPDispatchRuleDirect{
				RoomName: "room",
				Pin:      "1234",
			},
		}},
...
func TestSIPStoreTrunk(t *testing.T) {
	...
	oldList, err := rs.ListSIPTrunk(ctx, &conkit.ListSIPTrunkRequest{})
	require.NoError(t, err)
	...
	inList, err := rs.ListSIPInboundTrunk(ctx, &conkit.ListSIPInboundTrunkRequest{})
	require.NoError(t, err)
	...
	outList, err := rs.ListSIPOutboundTrunk(ctx, &conkit.ListSIPOutboundTrunkRequest{})
	require.NoError(t, err)
	...
	// Creation without ID should fail.
	oldT := &conkit.SIPTrunkInfo{
		Name: "Legacy",
	}
	err = rs.StoreSIPTrunk(ctx, oldT)
	require.Error(t, err)

	inT := &conkit.SIPInboundTrunkInfo{
		Name: "Inbound",
	}
	err = rs.StoreSIPInboundTrunk(ctx, inT)
	require.Error(t, err)

	outT := &conkit.SIPOutboundTrunkInfo{
		Name: "Outbound",
	}
	...
	listOld, err := rs.ListSIPTrunk(ctx, &conkit.ListSIPTrunkRequest{})
	require.NoError(t, err)
	require.Len(t, listOld.Items, 3)
	slices.SortFunc(listOld.Items, func(a, b *conkit.SIPTrunkInfo) int {
	...
	listIn, err := rs.ListSIPInboundTrunk(ctx, &conkit.ListSIPInboundTrunkRequest{})
	require.NoError(t, err)
	require.Len(t, listIn.Items, 2)
	slices.SortFunc(listIn.Items, func(a, b *conkit.SIPInboundTrunkInfo) int {
		return strings.Compare(a.Name, b.Name)
	})
	require.True(t, proto.Equal(inT, listIn.Items[0]))
	require.True(t, proto.Equal(oldT.AsInbound(), listIn.Items[1]))

	listOut, err := rs.ListSIPOutboundTrunk(ctx, &conkit.ListSIPOutboundTrunkRequest{})
	require.NoError(t, err)
	require.Len(t, listOut.Items, 2)
	slices.SortFunc(listOut.Items, func(a, b *conkit.SIPOutboundTrunkInfo) int {
		return strings.Compare(a.Name, b.Name)
	})
...
func TestSIPTrunkList(t *testing.T) {
	...
	testIter(t, func(ctx context.Context, id string) error {
		if strings.HasSuffix(id, "0") {
			return s.StoreSIPTrunk(ctx, &conkit.SIPTrunkInfo{
				SipTrunkId:     id,
				OutboundNumber: id,
			})
		}
		return s.StoreSIPInboundTrunk(ctx, &conkit.SIPInboundTrunkInfo{
			SipTrunkId: id,
			Numbers:    []string{id},
		})
	}, func(ctx context.Context, page *conkit.Pagination, ids []string) iters.PageIter[*conkit.SIPInboundTrunkInfo] {
		return conkit.ListPageIter(s.ListSIPInboundTrunk, &conkit.ListSIPInboundTrunkRequest{
...
func TestSIPRuleList(t *testing.T) {
	...
	testIter(t, func(ctx context.Context, id string) error {
		return s.StoreSIPDispatchRule(ctx, &conkit.SIPDispatchRuleInfo{
			SipDispatchRuleId: id,
			TrunkIds:          []string{id},
		})
	}, func(ctx context.Context, page *conkit.Pagination, ids []string) iters.PageIter[*conkit.SIPDispatchRuleInfo] {
		return conkit.ListPageIter(s.ListSIPDispatchRule, &conkit.ListSIPDispatchRuleRequest{
...
func testIter[T listItem](
	...
	list func(ctx context.Context, page *conkit.Pagination, ids []string) iters.PageIter[T],
) {
	ctx := context.Background()
	var all []string
	for i := 0; i < 250; i++ {
	...
	it = list(ctx, &conkit.Pagination{Limit: 10}, nil)
	got = allIDs(t, it)
	require.Equal(t, all, got)

	// List with pagination enabled, custom ID
	it = list(ctx, &conkit.Pagination{Limit: 10, AfterId: all[55]}, nil)
	got = allIDs(t, it)
	require.Equal(t, all[56:], got)

	// List fixed IDs
	it = list(ctx, &conkit.Pagination{Limit: 10, AfterId: all[5]}, []string{
		all[10],
		all[3],
		"invalid",
		all[8],
