...
func TestRoomInternal(t *testing.T) {
	...
	internal := &conkit.RoomInternal{
		TrackEgress: &conkit.AutoTrackEgress{Filepath: "egress"},
	}

	require.NoError(t, rs.StoreRoom(ctx, room, internal))
	actualRoom, actualInternal, err := rs.LoadRoom(ctx, conkit.RoomName(room.Name), true)
	...
	_, actualInternal, err = rs.LoadRoom(ctx, conkit.RoomName(room.Name), true)
	require.NoError(t, err)
...
func TestParticipantPersistence(t *testing.T) {
	...
	roomName := conkit.RoomName("room1")
	_ = rs.DeleteRoom(ctx, roomName)

	p := &conkit.ParticipantInfo{
		Sid:      "PA_test",
		Identity: "test",
		State:    conkit.ParticipantInfo_ACTIVE,
		Tracks: []*conkit.TrackInfo{
			{
				Sid:  "track1",
	...
	// result should match
	pGet, err := rs.LoadParticipant(ctx, roomName, conkit.ParticipantIdentity(p.Identity))
	require.NoError(t, err)
	...
	require.NoError(t, rs.DeleteParticipant(ctx, roomName, conkit.ParticipantIdentity(p.Identity)))
	...
	_, err = rs.LoadParticipant(ctx, roomName, conkit.ParticipantIdentity(p.Identity))
...
func TestRoomLock(t *testing.T) {
	...
	roomName := conkit.RoomName("myroom")
...
func TestEgressStore(t *testing.T) {
	...
	info := &conkit.EgressInfo{
		...
		RoomName: roomName,
		Status:   conkit.EgressStatus_EGRESS_STARTING,
		Request: &conkit.EgressInfo_RoomComposite{
			RoomComposite: &conkit.RoomCompositeEgressRequest{
				RoomName: roomName,
				Layout:   "speaker-dark",
			},
		},
	...
	// update
	info2.Status = conkit.EgressStatus_EGRESS_COMPLETE
	...
	// list by room
	list, err = rs.ListEgress(ctx, conkit.RoomName(roomName), false)
	require.NoError(t, err)
	require.Len(t, list, 1)

	// update
	info.Status = conkit.EgressStatus_EGRESS_COMPLETE
...
func TestAgentStore(t *testing.T) {
	...
	ad := &conkit.AgentDispatch{
		Id:        "dispatch_id",
		AgentName: "agent_name",
		Metadata:  "metadata",
		Room:      "room_name",
		State: &conkit.AgentDispatchState{
			CreatedAt: 1,
			DeletedAt: 2,
			Jobs: []*conkit.Job{
				&conkit.Job{
					...
					Metadata:  "metadata",
					AgentName: "agent_name",
					State: &conkit.JobState{
						Status:    conkit.JobStatus_JS_RUNNING,
						StartedAt: 3,
						EndedAt:   4,
						Error:     "error",
					},
	...
	expected.State.Jobs[0].Participant = &conkit.ParticipantInfo{
		Identity: "identity",
	}
	require.True(t, proto.Equal(expected, rd[0]))
...
func compareIngressInfo(t *testing.T, expected, v *conkit.IngressInfo) {
