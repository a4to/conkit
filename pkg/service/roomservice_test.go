...
import (
	...
	"github.com/a4to/protocol/rpc/rpcfakes"
...
func TestDeleteRoom(t *testing.T) {
	t.Run("missing permissions", func(t *testing.T) {
		...
		_, err := svc.DeleteRoom(ctx, &conkit.DeleteRoomRequest{
			Room: "testroom",
		})
		require.Error(t, err)
	})
...
func TestMetaDataLimits(t *testing.T) {
	t.Run("metadata exceed limits", func(t *testing.T) {
		...
		ctx := service.WithGrants(context.Background(), grant, "")
		_, err := svc.UpdateParticipant(ctx, &conkit.UpdateParticipantRequest{
			Room:     "testroom",
			Identity: "123",
			Metadata: "abcdefg",
		})
		...
		_, err = svc.UpdateRoomMetadata(ctx, &conkit.UpdateRoomMetadataRequest{
			Room:     "testroom",
			Metadata: "abcdefg",
		})
		terr, ok = err.(twirp.Error)
