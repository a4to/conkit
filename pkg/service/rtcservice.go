...
func (s *RTCService) validateInternal(r *http.Request) (conkit.RoomName, routing.ParticipantInit, int, error) {
	...
	roomName := conkit.RoomName(r.FormValue("room"))
	...
	createRequest := &conkit.CreateRoomRequest{
	...
	pi = routing.ParticipantInit{
		...
		ReconnectReason: conkit.ReconnectReason(reconnectReason),
		Identity:        conkit.ParticipantIdentity(claims.Identity),
		Name:            conkit.ParticipantName(claims.Name),
		AutoSubscribe:   true,
	...
	if pi.Reconnect {
		pi.ID = conkit.ParticipantID(participantID)
	}
...
func (s *RTCService) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	var initialResponse *conkit.SignalResponse
	...
	if !pi.Reconnect && initialResponse.GetJoin() != nil {
		pi.ID = conkit.ParticipantID(initialResponse.GetJoin().GetParticipant().GetSid())
	}
	...
	if pi.Reconnect && pi.ID != "" {
		signalStats.ResolveParticipant(&conkit.ParticipantInfo{
			Sid:      string(pi.ID),
	...
	go func() {
		...
		for {
			...
			case msg := <-cr.ResponseSource.ReadChan():
				...
				res, ok := msg.(*conkit.SignalResponse)
				if !ok {
					pLogger.Errorw(
				...
				case *conkit.SignalResponse_RoomUpdate:
	...
	for {
		...
		case *conkit.SignalRequest_Ping:
			count, perr := sigConn.WriteResponse(&conkit.SignalResponse{
				Message: &conkit.SignalResponse_Pong{
					//
		...
		case *conkit.SignalRequest_PingReq:
			count, perr := sigConn.WriteResponse(&conkit.SignalResponse{
				Message: &conkit.SignalResponse_PongResp{
					PongResp: &conkit.Pong{
...
func (s *RTCService) ParseClientInfo(r *http.Request) *conkit.ClientInfo {
	values := r.Form
	ci := &conkit.ClientInfo{}
	...
	switch sdkString {
	case "js":
		ci.Sdk = conkit.ClientInfo_JS
	case "ios", "swift":
		ci.Sdk = conkit.ClientInfo_SWIFT
	case "android":
		ci.Sdk = conkit.ClientInfo_ANDROID
	case "flutter":
		ci.Sdk = conkit.ClientInfo_FLUTTER
	case "go":
		ci.Sdk = conkit.ClientInfo_GO
	case "unity":
		ci.Sdk = conkit.ClientInfo_UNITY
	case "reactnative":
		ci.Sdk = conkit.ClientInfo_REACT_NATIVE
	case "rust":
		ci.Sdk = conkit.ClientInfo_RUST
	case "python":
		ci.Sdk = conkit.ClientInfo_PYTHON
	case "cpp":
		ci.Sdk = conkit.ClientInfo_CPP
	case "unityweb":
		ci.Sdk = conkit.ClientInfo_UNITY_WEB
	case "node":
		ci.Sdk = conkit.ClientInfo_NODE
	}
	...
	if ci.Sdk == conkit.ClientInfo_JS ||
		ci.Sdk == conkit.ClientInfo_REACT_NATIVE ||
		ci.Sdk == conkit.ClientInfo_FLUTTER ||
		ci.Sdk == conkit.ClientInfo_UNITY {
...
) (connectionResult, *conkit.SignalResponse, error) {
	var cr connectionResult
	var err error
...
func readInitialResponse(source routing.MessageSource, timeout time.Duration) (*conkit.SignalResponse, error) {
