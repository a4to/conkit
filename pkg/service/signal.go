...
import (
	...
	"github.com/a4to/psrpc/pkg/metadata"
...
func (s *defaultSessionHandler) HandleSession(
	...
	rtcNode, err := s.router.GetNodeForRoom(ctx, conkit.RoomName(pi.CreateRoom.Name))
	if err != nil {
		return err
	}

	if conkit.NodeID(rtcNode.Id) != s.currentNode.NodeID() {
...
func (r *signalService) RelaySignal(stream psrpc.ServerStream[*rpc.RelaySignalResponse, *rpc.RelaySignalRequest]) (err error) {
	...
	sink := routing.NewSignalMessageSink(routing.SignalSinkParams[*rpc.RelaySignalResponse, *rpc.RelaySignalRequest]{
		...
		ConnectionID: conkit.ConnectionID(ss.ConnectionId),
	})
	reqChan := routing.NewDefaultMessageChannel(conkit.ConnectionID(ss.ConnectionId))
	...
	err = r.sessionHandler.HandleSession(ctx, *pi, conkit.ConnectionID(ss.ConnectionId), reqChan, sink)
	if err != nil {
		sink.Close()
...
func (e signalResponseMessageWriter) Write(seq uint64, close bool, msgs []proto.Message) *rpc.RelaySignalResponse {
	r := &rpc.RelaySignalResponse{
		Seq:       seq,
		Responses: make([]*conkit.SignalResponse, 0, len(msgs)),
		Close:     close,
	}
	for _, m := range msgs {
		r.Responses = append(r.Responses, m.(*conkit.SignalResponse))
	}
	return r
}
