...
import (
	...
	"github.com/a4to/conkit-server/pkg/service/servicefakes"
...
func init() {
	prometheus.Init("node", conkit.NodeType_CONTROLLER)
}
...
func TestSignal(t *testing.T) {
	...
	t.Run("messages are delivered", func(t *testing.T) {
		...
		reqMessageIn := &conkit.SignalRequest{
			Message: &conkit.SignalRequest_Ping{Ping: 123},
		}
		resMessageIn := &conkit.SignalResponse{
			Message: &conkit.SignalResponse_Pong{Pong: 321},
		}
		...
		client, err := routing.NewSignalClient(conkit.NodeID("node0"), bus, cfg)
		...
		server, err := service.NewSignalServer(conkit.NodeID("node1"), "region", bus, cfg, handler)
		require.NoError(t, err)
