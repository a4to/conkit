...
func (h *TURNAuthHandler) CreateUsername(apiKey string, pID conkit.ParticipantID) string {
	return base62.EncodeToString([]byte(fmt.Sprintf("%s|%s", apiKey, pID)))
}

func (h *TURNAuthHandler) CreatePassword(apiKey string, pID conkit.ParticipantID) (string, error) {
...
func (h *TURNAuthHandler) HandleAuth(username, realm string, srcAddr net.Addr) (key []byte, ok bool) {
	...
	password, err := h.CreatePassword(parts[0], conkit.ParticipantID(parts[1]))
	if err != nil {
