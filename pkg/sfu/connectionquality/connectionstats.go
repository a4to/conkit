...
type ConnectionStats struct {
	...
	onStatsUpdate func(cs *ConnectionStats, stat *conkit.AnalyticsStat)
...
func (cs *ConnectionStats) OnStatsUpdate(fn func(cs *ConnectionStats, stat *conkit.AnalyticsStat)) {
	cs.onStatsUpdate = fn
}
...
func (cs *ConnectionStats) GetScoreAndQuality() (float32, conkit.ConnectionQuality) {
...
func (cs *ConnectionStats) getStat() {
	...
	if cs.onStatsUpdate != nil && len(streams) != 0 {
		analyticsStreams := make([]*conkit.AnalyticsStream, 0, len(streams))
		...
		if len(analyticsStreams) != 0 {
			cs.onStatsUpdate(cs, &conkit.AnalyticsStat{
				Score:   score,
...
func toAnalyticsVideoLayer(layer int32, layerStats *rtpstats.RTPDeltaInfo) *conkit.AnalyticsVideoLayer {
	if layerStats == nil {
		return nil
	}

	avl := &conkit.AnalyticsVideoLayer{
		Layer:   layer,
