...
var (
	qualityTransitionScore = map[conkit.ConnectionQuality]float64{
		conkit.ConnectionQuality_GOOD: 80,
		conkit.ConnectionQuality_POOR: 40,
		conkit.ConnectionQuality_LOST: 20,
	}
)
...
func (q *qualityScorer) updateMuteAtLocked(isMuted bool, at time.Time) {
	if isMuted {
		...
		if q.score != qualityTransitionScore[conkit.ConnectionQuality_LOST] {
			q.score = cMaxScore
		}
	} else {
		q.unmutedAt = at
...
func (q *qualityScorer) updateAtLocked(stat *windowStat, at time.Time) {
	...
	if stat.packets+stat.packetsPadding == 0 {
		if !stat.lastRTCPAt.IsZero() && at.Sub(stat.lastRTCPAt) > stat.duration {
			reason = "rtcp"
			score = qualityTransitionScore[conkit.ConnectionQuality_LOST]
		} else {
			reason = "dry"
			score = qualityTransitionScore[conkit.ConnectionQuality_POOR]
		}
	} else {
...
func (q *qualityScorer) GetScoreAndQuality() (float32, conkit.ConnectionQuality) {
	q.lock.RLock()
...
func (q *qualityScorer) GetMOSAndQuality() (float32, conkit.ConnectionQuality) {
	q.lock.RLock()
	defer q.lock.RUnlock()
...
func scoreToConnectionQuality(score float64) conkit.ConnectionQuality {
	// R-factor -> conkit.ConnectionQuality scale mapping roughly based on
	// https://www.itu.int/ITU-T/2005-2008/com12/emodelv1/tut.htm
	//
	// As there are only three levels in conkit.ConnectionQuality scale,
	...
	if score > qualityTransitionScore[conkit.ConnectionQuality_GOOD] {
		return conkit.ConnectionQuality_EXCELLENT
	}

	if score > qualityTransitionScore[conkit.ConnectionQuality_POOR] {
		return conkit.ConnectionQuality_GOOD
	}

	if score > qualityTransitionScore[conkit.ConnectionQuality_LOST] {
		return conkit.ConnectionQuality_POOR
	}

	return conkit.ConnectionQuality_LOST
}
