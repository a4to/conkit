...
import (
	...
	"github.com/a4to/protocol/utils/mono"

	"github.com/a4to/conkit-server/pkg/sfu/buffer"
	"github.com/a4to/conkit-server/pkg/sfu/ccutils"
	...
	act "github.com/a4to/conkit-server/pkg/sfu/rtpextension/abscapturetime"
	dd "github.com/a4to/conkit-server/pkg/sfu/rtpextension/dependencydescriptor"
	pd "github.com/a4to/conkit-server/pkg/sfu/rtpextension/playoutdelay"
	"github.com/a4to/conkit-server/pkg/sfu/rtpstats"
	"github.com/a4to/conkit-server/pkg/sfu/utils"
)
...
type TrackSender interface {
	...
	HandleRTCPSenderReportData(
		...
		isSVC bool,
		layer int32,
		publisherSRData *conkit.RTCPSenderReportState,
	) error
	Resync()
...
type DownTrackState struct {
	...
	ForwarderState                *conkit.RTPForwarderState
...
type DowntrackParams struct {
	...
	SubID                          conkit.ParticipantID
	StreamID                       string
	MaxTrack                       int
	PlayoutDelayLimit              *conkit.PlayoutDelay
	Pacer                          pacer.Pacer
...
type DownTrack struct {
	...
	cbMu                        sync.RWMutex
	onStatsUpdate               func(dt *DownTrack, stat *conkit.AnalyticsStat)
...
func NewDownTrack(params DowntrackParams) (*DownTrack, error) {
	...
	d.connectionStats.OnStatsUpdate(func(_cs *connectionquality.ConnectionStats, stat *conkit.AnalyticsStat) {
...
func (d *DownTrack) SubscriberID() conkit.ParticipantID {
	// add `createdAt` to ensure repeated subscriptions from same subscriber to same publisher does not collide
	return conkit.ParticipantID(fmt.Sprintf("%s:%d", d.params.SubID, d.createdAt))
}
...
func (d *DownTrack) OnStatsUpdate(fn func(dt *DownTrack, stat *conkit.AnalyticsStat)) {
	d.cbMu.Lock()
	defer d.cbMu.Unlock()
...
func (d *DownTrack) getOnStatsUpdate() func(dt *DownTrack, stat *conkit.AnalyticsStat) {
	d.cbMu.RLock()
	defer d.cbMu.RUnlock()
		...
		case *rtcp.ReceiverReport:
			...
			for _, r := range p.Reports {
				...
				if d.playoutDelay != nil {
					...
					if d.params.Source != conkit.TrackSource_SCREEN_SHARE {
...
func (d *DownTrack) GetConnectionScoreAndQuality() (float32, conkit.ConnectionQuality) {
	return d.connectionStats.GetScoreAndQuality()
}

func (d *DownTrack) GetTrackStats() *conkit.RTPStats {
...
func (d *DownTrack) handleRTCPSenderReportData(publisherSRData *conkit.RTCPSenderReportState, tsOffset uint64) {
