...
import (
	...
	"github.com/a4to/mediatransportutil"
	...
	"github.com/a4to/conkit-server/pkg/sfu/codecmunger"
	...
	"github.com/a4to/conkit-server/pkg/sfu/videolayerselector"
	"github.com/a4to/conkit-server/pkg/sfu/videolayerselector/temporallayerselector"
)
...
type refInfo struct {
	senderReport    *conkit.RTCPSenderReportState
	tsOffset        uint64
...
func (f *Forwarder) GetState() *conkit.RTPForwarderState {
	f.lock.RLock()
	...
	state := &conkit.RTPForwarderState{
		Started:                   f.started,
	...
	if vp8MungerState, ok := codecMungerState.(*conkit.VP8MungerState); ok {
		state.CodecMunger = &conkit.RTPForwarderState_Vp8Munger{
			Vp8Munger: vp8MungerState,
		}
	}

	state.SenderReportState = make([]*conkit.RTCPSenderReportState, len(f.refInfos))
...
func (f *Forwarder) SeedState(state *conkit.RTPForwarderState) {
...
func (f *Forwarder) SetRefSenderReport(isSVC bool, layer int32, srData *conkit.RTCPSenderReportState) {
	f.lock.Lock()
	defer f.lock.Unlock()
...
func (f *Forwarder) GetSenderReportParams() (int32, uint64, *conkit.RTCPSenderReportState) {
	f.lock.RLock()
	defer f.lock.RUnlock()
