...
import (
	...
	"github.com/a4to/mediatransportutil/pkg/bucket"
	...
	"github.com/a4to/conkit-server/pkg/sfu/streamtracker"
)
...
type TrackReceiver interface {
	...
	DeleteDownTrack(participantID conkit.ParticipantID)
...
type WebRTCReceiver struct {
	...
	onStatsUpdate    func(w *WebRTCReceiver, stat *conkit.AnalyticsStat)
...
func NewWebRTCReceiver(
	...
	w.connectionStats.OnStatsUpdate(func(_cs *connectionquality.ConnectionStats, stat *conkit.AnalyticsStat) {
...
func (w *WebRTCReceiver) TrackInfo() *conkit.TrackInfo {
	return w.trackInfo.Load()
}

func (w *WebRTCReceiver) UpdateTrackInfo(ti *conkit.TrackInfo) {
	w.trackInfo.Store(utils.CloneProto(ti))
	w.streamTrackerManager.UpdateTrackInfo(ti)
}

func (w *WebRTCReceiver) OnStatsUpdate(fn func(w *WebRTCReceiver, stat *conkit.AnalyticsStat)) {
	w.onStatsUpdate = fn
}
...
func (w *WebRTCReceiver) GetConnectionScoreAndQuality() (float32, conkit.ConnectionQuality) {
...
func (w *WebRTCReceiver) TrackID() conkit.TrackID {
	return w.trackID
}
...
func (w *WebRTCReceiver) notifyMaxExpectedLayer(layer int32) {
	...
	if w.Kind() == webrtc.RTPCodecTypeAudio || ti.Source == conkit.TrackSource_SCREEN_SHARE {
...
func (w *WebRTCReceiver) DeleteDownTrack(subscriberID conkit.ParticipantID) {
	if w.closed.Load() {
		return
	}
...
func (w *WebRTCReceiver) GetTrackStats() *conkit.RTPStats {
	w.bufferMu.RLock()
	defer w.bufferMu.RUnlock()

	stats := make([]*conkit.RTPStats, 0, len(w.buffers))
	for _, buff := range w.buffers {
		if buff == nil {
			continue
		}
