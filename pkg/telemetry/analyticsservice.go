...
type AnalyticsService interface {
	SendStats(ctx context.Context, stats []*conkit.AnalyticsStat)
	SendEvent(ctx context.Context, events *conkit.AnalyticsEvent)
	SendNodeRoomStates(ctx context.Context, nodeRooms *conkit.AnalyticsNodeRooms)
}
...
func (a *analyticsService) SendStats(_ context.Context, stats []*conkit.AnalyticsStat) {
	if a.stats == nil {
		return
	}
	...
	if err := a.stats.Send(&conkit.AnalyticsStats{Stats: stats}); err != nil {
		logger.Errorw("failed to send stats", err)
	}
}

func (a *analyticsService) SendEvent(_ context.Context, event *conkit.AnalyticsEvent) {
	if a.events == nil {
		return
	}
	...
	if err := a.events.Send(&conkit.AnalyticsEvents{
		Events: []*conkit.AnalyticsEvent{event},
	}); err != nil {
		logger.Errorw("failed to send event", err, "eventType", event.Type.String())
	}
}

func (a *analyticsService) SendNodeRoomStates(_ context.Context, nodeRooms *conkit.AnalyticsNodeRooms) {
	if a.nodeRooms == nil {
		return
	}
