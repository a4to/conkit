...
func Test_OnParticipantJoin_EventIsSent(t *testing.T) {
	...
	require.Equal(t, conkit.AnalyticsEventType_PARTICIPANT_JOINED, event.Type)
...
func Test_OnParticipantLeft_EventIsSent(t *testing.T) {
	...
	// do
	fixture.sut.ParticipantActive(context.Background(), room, participantInfo, &conkit.AnalyticsClientMeta{}, false)
	...
	require.Equal(t, conkit.AnalyticsEventType_PARTICIPANT_LEFT, event.Type)
	require.Equal(t, partSID, event.ParticipantId)
	require.Equal(t, room.Sid, event.RoomId)
	require.Equal(t, room, event.Room)
}
...
func Test_OnTrackUpdate_EventIsSent(t *testing.T) {
	...
	trackInfo := &conkit.TrackInfo{
		...
		Simulcast:  false,
		DisableDtx: false,
		Layers:     []*conkit.VideoLayer{layer},
	}

	// do
	fixture.sut.TrackPublishedUpdate(context.Background(), conkit.ParticipantID(partID), trackInfo)
	time.Sleep(time.Millisecond * 500)

	// test
	require.Equal(t, 1, fixture.analytics.SendEventCallCount())
	_, event := fixture.analytics.SendEventArgsForCall(0)
	require.Equal(t, conkit.AnalyticsEventType_TRACK_PUBLISHED_UPDATE, event.Type)
...
func Test_OnParticipantActive_EventIsSent(t *testing.T) {
	...
	// test
	// do
	clientMetaConnect := &conkit.AnalyticsClientMeta{
		ClientConnectTime: 420,
	}
	...
	require.Equal(t, conkit.AnalyticsEventType_PARTICIPANT_ACTIVE, eventActive.Type)
...
func Test_OnTrackSubscribed_EventIsSent(t *testing.T) {
	...
	room := &conkit.Room{Sid: "RoomSid", Name: "RoomName"}
	partSID := "part1"
	publisherInfo := &conkit.ParticipantInfo{Sid: "pub1", Identity: "publisher"}
	trackInfo := &conkit.TrackInfo{Sid: "tr1", Type: conkit.TrackType_VIDEO}
	...
	// do
	fixture.sut.TrackSubscribed(context.Background(), conkit.ParticipantID(partSID), trackInfo, publisherInfo, true)
	time.Sleep(time.Millisecond * 500)
	...
	require.Equal(t, conkit.AnalyticsEventType_TRACK_SUBSCRIBED, eventTrackSubscribed.Type)
