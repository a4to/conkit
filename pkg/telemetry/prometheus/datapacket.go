...
func initDataPacketStats(nodeID string, nodeType conkit.NodeType) {
...
func RecordDataPacketStream(h *conkit.DataStream_Header, destCount int) {
	streamType := "unknown"
	switch h.ContentHeader.(type) {
	case *conkit.DataStream_Header_TextHeader:
		streamType = "text"
	case *conkit.DataStream_Header_ByteHeader:
		streamType = "bytes"
	}
