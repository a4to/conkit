...
func initPacketStats(nodeID string, nodeType conkit.NodeType) {
...
func RecordPacketLoss(direction Direction, trackSource conkit.TrackSource, trackType conkit.TrackType, lost, total uint32) {
	if total > 0 {
...
func RecordPacketOutOfOrder(direction Direction, trackSource conkit.TrackSource, trackType conkit.TrackType, ooo, total uint32) {
	if total > 0 {
...
func RecordJitter(direction Direction, trackSource conkit.TrackSource, trackType conkit.TrackType, jitter uint32) {
	if jitter > 0 {
		promJitter.WithLabelValues(string(direction), trackSource.String(), trackType.String()).Observe(float64(jitter))
	}
}

func RecordRTT(direction Direction, trackSource conkit.TrackSource, trackType conkit.TrackType, rtt uint32) {
	if rtt > 0 {
