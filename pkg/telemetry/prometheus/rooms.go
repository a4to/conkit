...
func initRoomStats(nodeID string, nodeType conkit.NodeType) {
	promRoomCurrent = prometheus.NewGauge(prometheus.GaugeOpts{
		Namespace:   conkitNamespace,
		Subsystem:   "room",
		Name:        "total",
...
func RecordPublishTime(source conkit.TrackSource, trackType conkit.TrackType, d time.Duration, sdk conkit.ClientInfo_SDK, kind conkit.ParticipantInfo_Kind) {
	recordPubSubTime(true, source, trackType, d, sdk, kind, 1)
}

func RecordSubscribeTime(source conkit.TrackSource, trackType conkit.TrackType, d time.Duration, sdk conkit.ClientInfo_SDK, kind conkit.ParticipantInfo_Kind, count int) {
	recordPubSubTime(false, source, trackType, d, sdk, kind, count)
}

func recordPubSubTime(isPublish bool, source conkit.TrackSource, trackType conkit.TrackType, d time.Duration, sdk conkit.ClientInfo_SDK, kind conkit.ParticipantInfo_Kind, count int) {
	direction := "subscribe"
	if isPublish {
		direction = "publish"
	}
