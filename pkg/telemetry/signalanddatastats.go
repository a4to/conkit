...
func NewBytesTrackStats(trackID conkit.TrackID, pID conkit.ParticipantID, telemetry TelemetryService) *BytesTrackStats {
	s := &BytesTrackStats{
		trackID:   trackID,
		pID:       pID,
		telemetry: telemetry,
...
func (s *BytesTrackStats) report() {
	if recv := s.recv.Swap(0); recv > 0 {
		s.telemetry.TrackStats(StatsKeyForData(conkit.StreamType_UPSTREAM, s.pID, s.trackID), &conkit.AnalyticsStat{
	...
	if send := s.send.Swap(0); send > 0 {
		s.telemetry.TrackStats(StatsKeyForData(conkit.StreamType_DOWNSTREAM, s.pID, s.trackID), &conkit.AnalyticsStat{
			Streams: []*conkit.AnalyticsStream{
				{
					PrimaryBytes:   send,
...
func (s *BytesSignalStats) ResolveRoom(ri *conkit.Room) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *BytesSignalStats) ResolveParticipant(pi *conkit.ParticipantInfo) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.pi == nil && pi != nil {
		s.pi = &conkit.ParticipantInfo{
			Sid:      pi.Sid,
...
func (s *BytesSignalStats) maybeStart() {
	...
	s.pID = conkit.ParticipantID(s.pi.Sid)
...
func BytesTrackIDForParticipantID(typ BytesTrackType, participantID conkit.ParticipantID) conkit.TrackID {
	return conkit.TrackID(fmt.Sprintf("%s_%s%s", utils.TrackPrefix, string(typ), participantID))
}
