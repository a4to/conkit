...
func init() {
	prometheus.Init("test", conkit.NodeType_SERVER)
}
...
func Test_ParticipantAndRoomDataAreSentWithAnalytics(t *testing.T) {
	...
	// prepare
	room := &conkit.Room{Sid: "RoomSid", Name: "RoomName"}
	partSID := conkit.ParticipantID("part1")
	clientInfo := &conkit.ClientInfo{Sdk: 2}
	participantInfo := &conkit.ParticipantInfo{Sid: string(partSID)}
	fixture.sut.ParticipantJoined(context.Background(), room, participantInfo, clientInfo, nil, true)

	// do
	packet := 33
	stat := &conkit.AnalyticsStat{Streams: []*conkit.AnalyticsStream{{PrimaryBytes: uint64(packet)}}}
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_DOWNSTREAM, partSID, ""), stat)
	...
	require.Equal(t, conkit.StreamType_DOWNSTREAM, stats[0].Kind)
...
func Test_OnDownstreamPackets(t *testing.T) {
	...
	trackID := conkit.TrackID("trackID")
	for i := range packets {
		stat := &conkit.AnalyticsStat{Streams: []*conkit.AnalyticsStream{{PrimaryBytes: uint64(packets[i]), PrimaryPackets: uint32(1)}}}
		fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_DOWNSTREAM, partSID, trackID), stat)
	}
...
func Test_OnDownstreamPackets_SeveralTracks(t *testing.T) {
	...
	// do
	packet1 := 33
	trackID1 := conkit.TrackID("trackID1")
	stat1 := &conkit.AnalyticsStat{Streams: []*conkit.AnalyticsStream{{PrimaryBytes: uint64(packet1), PrimaryPackets: 1}}}
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_DOWNSTREAM, partSID, trackID1), stat1)

	packet2 := 23
	trackID2 := conkit.TrackID("trackID2")
	stat2 := &conkit.AnalyticsStat{Streams: []*conkit.AnalyticsStream{{PrimaryBytes: uint64(packet2), PrimaryPackets: 1}}}
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_DOWNSTREAM, partSID, trackID2), stat2)
...
func Test_OnDownStreamStat(t *testing.T) {
	...
	// do
	stat1 := &conkit.AnalyticsStat{
		Streams: []*conkit.AnalyticsStream{
			{
				PrimaryBytes:   1,
				PrimaryPackets: 1,
				PacketsLost:    3,
	...
	trackID := conkit.TrackID("trackID1")
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_DOWNSTREAM, partSID, trackID), stat1)

	stat2 := &conkit.AnalyticsStat{
		Streams: []*conkit.AnalyticsStream{
			{
				PrimaryBytes:   2,
				PrimaryPackets: 2,
	...
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_DOWNSTREAM, partSID, trackID), stat2)
...
func Test_PacketLostDiffShouldBeSentToTelemetry(t *testing.T) {
	...
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_DOWNSTREAM, partSID, trackID), stat1) // there should be bytes reported so that stats are sent
...
func Test_OnDownStreamRTCP_SeveralTracks(t *testing.T) {
	...
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_DOWNSTREAM, partSID, trackID1), stat1) // there should be bytes reported so that stats are sent
	...
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_DOWNSTREAM, partSID, trackID1), stat2)

	stat3 := &conkit.AnalyticsStat{
		Streams: []*conkit.AnalyticsStream{
			{
				PrimaryBytes:   3,
				PrimaryPackets: 3,
	...
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_DOWNSTREAM, partSID, trackID2), stat3)
	...
	for _, sentStat := range stats {
		if conkit.TrackID(sentStat.TrackId) == trackID1 {
			found1 = true
			require.Equal(t, conkit.StreamType_DOWNSTREAM, sentStat.Kind)
...
func Test_OnUpstreamStat(t *testing.T) {
	...
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_UPSTREAM, partSID, trackID), stat1)
	...
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_UPSTREAM, partSID, trackID), stat2)
	...
	_, stats := fixture.analytics.SendStatsArgsForCall(0)
	require.Equal(t, 1, len(stats))
	require.Equal(t, conkit.StreamType_UPSTREAM, stats[0].Kind)
	require.Equal(t, 2, int(stats[0].Streams[0].Nacks))
	require.Equal(t, 2, int(stats[0].Streams[0].Plis))
...
func Test_OnUpstreamRTCP_SeveralTracks(t *testing.T) {
	...
	room := &conkit.Room{}
	partSID := conkit.ParticipantID("part1")
	identity := conkit.ParticipantIdentity("part1Identity")
	participantInfo := &conkit.ParticipantInfo{Sid: string(partSID), Identity: string(identity)}
	fixture.sut.ParticipantJoined(context.Background(), room, participantInfo, nil, nil, true)
	...
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_UPSTREAM, partSID, trackID1), stat1)
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_UPSTREAM, partSID, trackID2), stat1) // using same buffer is not correct but for test it is fine
	...
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_UPSTREAM, partSID, trackID1), stat2)
	...
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_UPSTREAM, partSID, trackID2), stat3)
	...
	for _, sentStat := range stats {
		if conkit.TrackID(sentStat.TrackId) == trackID1 {
			found1 = true
			require.Equal(t, conkit.StreamType_UPSTREAM, sentStat.Kind)
			require.Equal(t, 1, int(sentStat.Streams[0].Nacks)) // see pkts1 above
		} else if conkit.TrackID(sentStat.TrackId) == trackID2 {
			found2 = true
			require.Equal(t, conkit.StreamType_UPSTREAM, sentStat.Kind)
	...
	fixture.sut.TrackUnpublished(context.Background(), partSID, identity, &conkit.TrackInfo{Sid: string(trackID2)}, true)
...
func Test_AnalyticsSentWhenParticipantLeaves(t *testing.T) {
	...
	room := &conkit.Room{}
	partSID := "part1"
	participantInfo := &conkit.ParticipantInfo{Sid: partSID}
	fixture.sut.ParticipantJoined(context.Background(), room, participantInfo, nil, nil, true)
...
func Test_AddUpTrack(t *testing.T) {
	...
	trackID := conkit.TrackID("trackID")
	fixture.sut.TrackStats(telemetry.StatsKeyForData(conkit.StreamType_UPSTREAM, partSID, trackID), stat)
...
func Test_BothDownstreamAndUpstreamStatsAreSentTogether(t *testing.T) {
	...
	require.Equal(t, 2, len(stats))
	require.Equal(t, conkit.StreamType_UPSTREAM, stats[0].Kind)
	require.Equal(t, conkit.StreamType_DOWNSTREAM, stats[1].Kind)
}
