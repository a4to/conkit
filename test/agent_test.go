...
func TestAgents(t *testing.T) {
	...
	defer ac5.close()
	defer ac6.close()
	ac1.Run(conkit.JobType_JT_ROOM, "default")
	ac2.Run(conkit.JobType_JT_ROOM, "default")
	ac3.Run(conkit.JobType_JT_PUBLISHER, "default")
	ac4.Run(conkit.JobType_JT_PUBLISHER, "default")
	ac5.Run(conkit.JobType_JT_PARTICIPANT, "default")
	ac6.Run(conkit.JobType_JT_PARTICIPANT, "default")
...
func TestAgentNamespaces(t *testing.T) {
	...
	defer ac1.close()
	defer ac2.close()
	ac1.Run(conkit.JobType_JT_ROOM, "namespace1")
	ac2.Run(conkit.JobType_JT_ROOM, "namespace2")

	_, err = roomClient.CreateRoom(contextWithToken(createRoomToken()), &conkit.CreateRoomRequest{
		Name: testRoom,
		Agents: []*conkit.RoomAgentDispatch{
			{},
			{
				AgentName: "ag",
			},
...
func TestAgentMultiNode(t *testing.T) {
	...
	defer ac2.close()
	ac1.Run(conkit.JobType_JT_ROOM, "default")
	ac2.Run(conkit.JobType_JT_PUBLISHER, "default")
