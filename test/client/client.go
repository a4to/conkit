...
import (
	...
	"github.com/a4to/conkit-server/pkg/rtc/transport/transportfakes"
...
type SignalRequestHandler func(msg *conkit.SignalRequest) error
type SignalRequestInterceptor func(msg *conkit.SignalRequest, next SignalRequestHandler) error
type SignalResponseHandler func(msg *conkit.SignalResponse) error
type SignalResponseInterceptor func(msg *conkit.SignalResponse, next SignalResponseHandler) error
...
type RTCClient struct {
	...
	subscribedTracks   map[conkit.ParticipantID][]*webrtc.TrackRemote
	localParticipant   *conkit.ParticipantInfo
	remoteParticipants map[conkit.ParticipantID]*conkit.ParticipantInfo
	...
	pendingPublishedTracks map[string]*conkit.TrackInfo
	...
	// map of conkit.ParticipantID and last packet
	lastPackets   map[conkit.ParticipantID]*rtp.Packet
	bytesReceived map[conkit.ParticipantID]uint64

	subscriptionResponse atomic.Pointer[conkit.SubscriptionResponse]
}
...
func NewWebSocketConn(host, token string, opts *Options) (*websocket.Conn, error) {
	...
	if opts != nil {
		...
		if opts.ClientInfo != nil {
			...
			if opts.ClientInfo.Sdk != conkit.ClientInfo_UNKNOWN {
...
func NewRTCClient(conn *websocket.Conn, opts *Options) (*RTCClient, error) {
	...
	c := &RTCClient{
		...
		pendingPublishedTracks: make(map[string]*conkit.TrackInfo),
		subscribedTracks:       make(map[conkit.ParticipantID][]*webrtc.TrackRemote),
		remoteParticipants:     make(map[conkit.ParticipantID]*conkit.ParticipantInfo),
		me:                     &webrtc.MediaEngine{},
		lastPackets:            make(map[conkit.ParticipantID]*rtp.Packet),
		bytesReceived:          make(map[conkit.ParticipantID]uint64),
	}
	...
	var codecs []*conkit.Codec
	for _, codec := range []*conkit.Codec{
		{
			Mime: "audio/opus",
		},
		{
	...
	publisherHandler.OnICECandidateCalls(func(ic *webrtc.ICECandidate, t conkit.SignalTarget) error {
		return c.SendIceCandidate(ic, conkit.SignalTarget_PUBLISHER)
	})
	...
	subscriberHandler.OnICECandidateCalls(func(ic *webrtc.ICECandidate, t conkit.SignalTarget) error {
		if ic == nil {
			return nil
		}
		return c.SendIceCandidate(ic, conkit.SignalTarget_SUBSCRIBER)
	})
	...
	subscriberHandler.OnAnswerCalls(func(answer webrtc.SessionDescription) error {
		...
		return c.SendRequest(&conkit.SignalRequest{
			Message: &conkit.SignalRequest_Answer{
...
func (c *RTCClient) ID() conkit.ParticipantID {
	return c.id
}
...
func (c *RTCClient) handleSignalResponse(res *conkit.SignalResponse) error {
	switch msg := res.Message.(type) {
	case *conkit.SignalResponse_Join:
		c.localParticipant = msg.Join.Participant
		c.id = conkit.ParticipantID(msg.Join.Participant.Sid)
		c.lock.Lock()
		for _, p := range msg.Join.OtherParticipants {
			c.remoteParticipants[conkit.ParticipantID(p.Sid)] = p
		}
		c.lock.Unlock()
	...
	case *conkit.SignalResponse_TrackPublished:
		...
		c.lock.Unlock()
	case *conkit.SignalResponse_RefreshToken:
		c.lock.Lock()
		c.refreshToken = msg.RefreshToken
		c.lock.Unlock()
	case *conkit.SignalResponse_TrackUnpublished:
		...
		c.lock.Unlock()
	case *conkit.SignalResponse_Pong:
		c.pongReceivedAt.Store(msg.Pong)
	case *conkit.SignalResponse_SubscriptionResponse:
...
func (c *RTCClient) ReadResponse() (*conkit.SignalResponse, error) {
	for {
...
func (c *RTCClient) SubscribedTracks() map[conkit.ParticipantID][]*webrtc.TrackRemote {
	// create a copy of this
	c.lock.Lock()
	defer c.lock.Unlock()
	tracks := make(map[conkit.ParticipantID][]*webrtc.TrackRemote, len(c.subscribedTracks))
...
func (c *RTCClient) RemoteParticipants() []*conkit.ParticipantInfo {
	c.lock.Lock()
	defer c.lock.Unlock()
	return funk.Values(c.remoteParticipants).([]*conkit.ParticipantInfo)
}

func (c *RTCClient) GetRemoteParticipant(sid conkit.ParticipantID) *conkit.ParticipantInfo {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *RTCClient) Stop() {
	...
	_ = c.SendRequest(&conkit.SignalRequest{
		Message: &conkit.SignalRequest_Leave{
			Leave: &conkit.LeaveRequest{
				Reason: conkit.DisconnectReason_CLIENT_INITIATED,
				Action: conkit.LeaveRequest_DISCONNECT,
			},
		},
	})
...
func (c *RTCClient) GetSubscriptionResponseAndClear() *conkit.SubscriptionResponse {
	return c.subscriptionResponse.Swap(nil)
}

func (c *RTCClient) SendPing() error {
	return c.SendRequest(&conkit.SignalRequest{
		Message: &conkit.SignalRequest_Ping{
			Ping: time.Now().UnixNano(),
		},
	})
}

func (c *RTCClient) SendRequest(msg *conkit.SignalRequest) error {
...
func (c *RTCClient) SendIceCandidate(ic *webrtc.ICECandidate, target conkit.SignalTarget) error {
	...
	return c.SendRequest(&conkit.SignalRequest{
		Message: &conkit.SignalRequest_Trickle{
...
func (c *RTCClient) SetAttributes(attrs map[string]string) error {
	return c.SendRequest(&conkit.SignalRequest{
		Message: &conkit.SignalRequest_UpdateMetadata{
			UpdateMetadata: &conkit.UpdateParticipantMetadata{
				Attributes: attrs,
			},
		},
	})
...
func (c *RTCClient) AddTrack(track *webrtc.TrackLocalStaticSample, path string, opts ...AddTrackOption) (writer *TrackWriter, err error) {
	...
	trackType := conkit.TrackType_AUDIO
	if track.Kind() == webrtc.RTPCodecTypeVideo {
		trackType = conkit.TrackType_VIDEO
	}
...
func (c *RTCClient) SendAddTrack(cid string, name string, trackType conkit.TrackType) error {
	return c.SendRequest(&conkit.SignalRequest{
		Message: &conkit.SignalRequest_AddTrack{
			AddTrack: &conkit.AddTrackRequest{
				Cid:  cid,
				Name: name,
				Type: trackType,
			},
...
func (c *RTCClient) PublishData(data []byte, kind conkit.DataPacket_Kind) error {
	...
	dpData, err := proto.Marshal(&conkit.DataPacket{
		Value: &conkit.DataPacket_User{
			User: &conkit.UserPacket{Payload: data},
		},
	})
	if err != nil {
		return err
...
func (c *RTCClient) handleDataMessage(kind conkit.DataPacket_Kind, data []byte) {
	...
	dp.Kind = kind
	if val, ok := dp.Value.(*conkit.DataPacket_User); ok {
...
func (c *RTCClient) onOffer(offer webrtc.SessionDescription) error {
	...
	return c.SendRequest(&conkit.SignalRequest{
		Message: &conkit.SignalRequest_Offer{
...
func (c *RTCClient) processTrack(track *webrtc.TrackRemote) {
	...
	if trackId == "" {
		trackId = conkit.TrackID(track.ID())
	}
	c.lock.Lock()
