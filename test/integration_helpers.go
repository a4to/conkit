...
var roomClient conkit.RoomService
...
func createSingleNodeServer(configUpdater func(*config.Config)) *service.LivekitServer {
	...
	currentNode.SetNodeID(conkit.NodeID(guid.New(nodeID1)))
	...
	roomClient = conkit.NewRoomServiceJSONClient(fmt.Sprintf("http://localhost:%d", defaultServerPort), &http.Client{})
	return s
}
...
func createMultiNodeServer(nodeID string, port uint32) *service.LivekitServer {
	...
	currentNode.SetNodeID(conkit.NodeID(nodeID))
	...
	roomClient = conkit.NewRoomServiceJSONClient(fmt.Sprintf("http://localhost:%d", port), &http.Client{})
	return s
}
