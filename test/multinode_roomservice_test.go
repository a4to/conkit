...
func TestMultiNodeUpdateRoomMetadata(t *testing.T) {
	...
	t.Run("when room is empty", func(t *testing.T) {
		...
		rm, err := roomClient.UpdateRoomMetadata(contextWithToken(adminRoomToken("emptyRoom")), &conkit.UpdateRoomMetadataRequest{
			Room:     "emptyRoom",
...
func TestMultiNodeRemoveParticipant(t *testing.T) {
	...
	_, err := roomClient.RemoveParticipant(ctx, &conkit.RoomParticipantIdentity{
		Room:     testRoom,
	...
	listRes, err := roomClient.ListParticipants(ctx, &conkit.ListParticipantsRequest{
		Room: testRoom,
	})
	require.NoError(t, err)
...
func TestMultiNodeUpdateParticipantMetadata(t *testing.T) {
	...
	ctx := contextWithToken(adminRoomToken(testRoom))
	res, err := roomClient.UpdateParticipant(ctx, &conkit.UpdateParticipantRequest{
		Room:     testRoom,
...
func TestMultiNodeMutePublishedTrack(t *testing.T) {
	...
	testutils.WithTimeout(t, func() string {
		res, err := roomClient.GetParticipant(ctx, &conkit.RoomParticipantIdentity{
			Room:     testRoom,
			Identity: identity,
		})
		require.NoError(t, err)
	...
	res, err := roomClient.MutePublishedTrack(ctx, &conkit.MuteRoomTrackRequest{
		Room:     testRoom,
		Identity: identity,
		TrackSid: trackIDs[0],
		Muted:    true,
