...
import (
	...
	"github.com/a4to/conkit-server/test/client"
)
...
func TestMultiNodeRouting(t *testing.T) {
	...
	_, err := roomClient.CreateRoom(contextWithToken(createRoomToken()), &conkit.CreateRoomRequest{
		Name: testRoom,
	})
...
func TestMultiNodeUpdateAttributes(t *testing.T) {
	...
	_, err := roomClient.UpdateParticipant(contextWithToken(adminRoomToken(testRoom)), &conkit.UpdateParticipantRequest{
		Room:     testRoom,
		Identity: "au1",
...
func TestCloseDisconnectedParticipantOnSignalClose(t *testing.T) {
	...
	c2 := createRTCClient("c2", defaultServerPort, &client.Options{
		SignalRequestInterceptor: func(msg *conkit.SignalRequest, next client.SignalRequestHandler) error {
			switch msg.Message.(type) {
			case *conkit.SignalRequest_Offer, *conkit.SignalRequest_Answer, *conkit.SignalRequest_Leave:
				return nil
			default:
				return next(msg)
			}
		},
		SignalResponseInterceptor: func(msg *conkit.SignalResponse, next client.SignalResponseHandler) error {
			switch msg.Message.(type) {
			case *conkit.SignalResponse_Offer, *conkit.SignalResponse_Answer:
				return nil
			default:
				return next(msg)
			}
