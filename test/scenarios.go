...
func scenarioDataPublish(t *testing.T) {
	...
	c2.OnDataReceived = func(data []byte, sid string) {
		if string(data) == payload && conkit.ParticipantID(sid) == c1.ID() {
			received.Store(true)
		}
	}

	require.NoError(t, c1.PublishData([]byte(payload), conkit.DataPacket_RELIABLE))
...
func scenarioJoinClosedRoom(t *testing.T) {
	...
	_, err := roomClient.DeleteRoom(contextWithToken(createRoomToken()), &conkit.DeleteRoomRequest{
		Room: testRoom,
	})
...
func closeNonRTCRoom(t *testing.T) {
	...
	_, err := roomClient.CreateRoom(createCtx, &conkit.CreateRoomRequest{
		Name: testRoom,
	})
	require.NoError(t, err)

	_, err = roomClient.DeleteRoom(createCtx, &conkit.DeleteRoomRequest{
		Room: testRoom,
	})
	require.NoError(t, err)
}
...
func roomServiceListRoom(t *testing.T) {
	...
	t.Run("list all rooms", func(t *testing.T) {
		res, err := roomClient.ListRooms(listCtx, &conkit.ListRoomsRequest{})
		require.NoError(t, err)
