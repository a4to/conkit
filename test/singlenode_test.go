...
import (
	...
	testclient "github.com/a4to/conkit-server/test/client"
)
...
func TestSinglePublisher(t *testing.T) {
	...
	audioTrack := funk.Find(remoteC1.Tracks, func(ti *conkit.TrackInfo) bool {
...
func TestSingleNodeUpdateParticipant(t *testing.T) {
	...
	t.Run("update nonexistent participant", func(t *testing.T) {
		_, err := roomClient.UpdateParticipant(adminCtx, &conkit.UpdateParticipantRequest{
			Room:     testRoom,
			Identity: "nonexistent",
			Permission: &conkit.ParticipantPermission{
				CanPublish: true,
			},
		})
		require.Error(t, err)
...
func TestSingleNodeDoubleSlash(t *testing.T) {
	...
	roomClient = conkit.NewRoomServiceJSONClient(fmt.Sprintf("http://localhost:%d/", s.HTTPPort()), &http.Client{})
	_, err := roomClient.ListRooms(contextWithToken(listRoomToken()), &conkit.ListRoomsRequest{})
	require.NoError(t, err)
}
...
func TestAutoCreate(t *testing.T) {
	...
	t.Run("join with explicit createRoom", func(t *testing.T) {
		...
		// explicitly create
		_, err := roomClient.CreateRoom(contextWithToken(createRoomToken()), &conkit.CreateRoomRequest{Name: testRoom})
		require.NoError(t, err)
...
func TestSingleNodeUpdateSubscriptionPermissions(t *testing.T) {
	...
	_, err = roomClient.UpdateParticipant(ctx, &conkit.UpdateParticipantRequest{
		Room:     testRoom,
		Identity: "sub",
		Permission: &conkit.ParticipantPermission{
			CanSubscribe: true,
...
func TestSubscribeToCodecUnsupported(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		...
		require.Equal(t, conkit.SubscriptionError_SE_CODEC_UNSUPPORTED, sr.Err)
		return true
...
func TestDataPublishSlowSubscriber(t *testing.T) {
	...
	go func() {
		...
		for !stopWrite.Load() {
			...
			if err := pub.PublishData(buf, conkit.DataPacket_RELIABLE); err != nil {
