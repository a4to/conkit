...
type webhookTestServer struct {
	server   *http.Server
	events   map[string]*conkit.WebhookEvent
	lock     sync.Mutex
...
func newTestServer(addr string) *webhookTestServer {
	s := &webhookTestServer{
		events:   make(map[string]*conkit.WebhookEvent),
...
func (s *webhookTestServer) GetEvent(name string) *conkit.WebhookEvent {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *webhookTestServer) ClearEvents() {
	s.lock.Lock()
	s.events = make(map[string]*conkit.WebhookEvent)
	s.lock.Unlock()
}
